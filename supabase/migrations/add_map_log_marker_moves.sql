-- ============================================================================
-- Map log: marker moves + trail completion linked to the start entry
-- ============================================================================
-- * marker.moved events: captures a marker being relocated via the move
--   function (drag + place). details carry from_lng/from_lat (where it was)
--   and lng/lat (where it is now) so the feed can offer a "Go" button and a
--   "Moved from" row. Sub-0.1 m jitter is ignored (float churn guard) and
--   the trigger now watches marker_data as a whole, deciding inside the
--   function what is meaningful.
-- * trail completion: when a trail closes, its original trail.started row is
--   updated with completed = true (plus duration / distance / area) so the
--   "Started trailing" entry doubles as the completed-trail entry the UI can
--   hang a "Show trail" replay button off. auto_closed flags system closes.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Marker capture: placed / deleted / restored / edited / moved
-- ---------------------------------------------------------------------------
...
begin
    if tg_op = 'DELETE' then
        ...
        -- Change on a live marker — work out what actually happened. The
        -- trigger fires on any marker_data change; silo fill, photos, notes,
        -- sync churn etc. fall through and are not logged.
        v_row := new;
        v_props := coalesce(new.marker_data -> 'properties', '{}'::jsonb);
        v_from_props := coalesce(old.marker_data -> 'properties', '{}'::jsonb);
        v_coords := new.marker_data -> 'geometry' -> 'coordinates';
        v_from_coords := old.marker_data -> 'geometry' -> 'coordinates';

        if greatest(
               abs(coalesce((v_coords ->> 0)::double precision, 0)
                   - coalesce((v_from_coords ->> 0)::double precision, 0)),
               abs(coalesce((v_coords ->> 1)::double precision, 0)
                   - coalesce((v_from_coords ->> 1)::double precision, 0))
           ) > 0.000001 then   -- ~0.1 m: real moves only, not float noise
            v_action := 'marker.moved';
            v_summary := 'Moved a marker';
        elsif (v_from_props ->> 'icon') is distinct from (v_props ->> 'icon')
           or (v_from_props ->> 'marker_color') is distinct from (v_props ->> 'marker_color')
           or (v_from_props ->> 'tint_mode') is distinct from (v_props ->> 'tint_mode') then
    ...
    -- Current coordinates for every action (placed/deleted/restored too).
    v_coords := coalesce(v_coords, v_row.marker_data -> 'geometry' -> 'coordinates');
    ...
    values (
        ...
        jsonb_strip_nulls(
            ...
            || case
                   when v_action = 'marker.edited' then jsonb_build_object(
                       'from_icon',         v_from_props -> 'icon',
                       'from_marker_color', v_from_props -> 'marker_color',
                       'from_tint_mode',    v_from_props -> 'tint_mode'
                   )
                   when v_action = 'marker.moved' then jsonb_build_object(
                       'from_lng', v_from_coords -> 0,
                       'from_lat', v_from_coords -> 1
                   )
                   else '{}'::jsonb
               end
        )
...
drop trigger if exists map_log_marker_updated on public.map_markers;
create trigger map_log_marker_updated
    after update on public.map_markers
    for each row
    when (new.deleted is distinct from old.deleted
          or new.marker_data is distinct from old.marker_data)
...
begin
    ...
    if new.end_time is not null and old.end_time is null then
        ...
        -- Link the completion back onto the "Started trailing" entry so the
        -- feed can show it as completed (with metrics + a replay button).
        update public.map_log ml
           set details = ml.details || jsonb_strip_nulls(jsonb_build_object(
                   'completed',    true,
                   'auto_closed',  v_auto,
                   'closed_at',    new.end_time,
                   'duration_s',   v_duration_s,
                   'trail_distance', new.trail_distance,
                   'trail_hectares', new.trail_hectares
               ))
         where ml.entity_type = 'trail'
           and ml.entity_id = new.id
           and ml.action = 'trail.started';
        return null;
    end if;
    ...
    if new.metrics_calculated is true
       ...
       and new.end_time is not null then
        ...
        -- Same metrics onto the started entry (it shows them too now).
        update public.map_log ml
           set details = ml.details || jsonb_strip_nulls(jsonb_build_object(
                   'trail_distance', new.trail_distance,
                   'trail_hectares', new.trail_hectares,
