...
export const POST: RequestHandler = async ({ locals, request }) => {
  try {
    ...
    const { data: otherExplain, error: otherExplainError } =
    ...
    const { data: otherTrailData, error: otherError } =
      await locals.supabase.rpc("get_other_trail_data", {
        p_master_map_id: masterMapId,
        p_user_id: userId,
        p_retention_timestamp: retentionTimestamp,
      })
    ...
    return json(
      { error: "An error occurred while fetching trail data" },
      { status: 500 },
    )
  }
}
