// src/routes/api/map-trails/check-open-trails/+server.ts
...
export const POST: RequestHandler = async ({ request, locals }) => {
    ...
    try {
        ...
        const { mostRecentTrail, processedTrails, staleTrailClosed, lastActivity } =
            result

        // If any trail was closed due to being stale, consider all trails closed
        if (staleTrailClosed) {
            console.log(
                "Trail was closed due to being stale, considering all trails inactive",
            )
            return json({
                openTrail: null,
                trailData: null,
