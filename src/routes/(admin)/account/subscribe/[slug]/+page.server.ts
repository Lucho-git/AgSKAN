...
async function listCoupons() {
  try {
    ...
    coupons.data.forEach((coupon) => {
      console.log(
        `ID: ${coupon.id}, Name: ${coupon.name}, Active: ${coupon.valid}`,
      )
    })
    return coupons
  } catch (e) {
...
export const load: PageServerLoad = async ({
  ...
  try {
    ...
    const isValidOnboardingPath = onboardingPaths.some((path) =>
      safeReferrer.startsWith(path),
    )
    ...
    if (
      ...
      e.status === 303 &&
      !e.headers.get("location")?.includes("checkout.stripe.com")
    ) {
      throw e
    }
