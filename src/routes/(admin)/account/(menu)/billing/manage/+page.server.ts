...
export const load: PageServerLoad = async ({
  ...
  if (subError) {
    throw error(500, {
      message:
        "Error fetching subscription. If issue persists, please contact us.",
    })
  }
...
export const actions = {
      ...
      const { data, error: updateError } = await supabaseServiceRole
        ...
        .update({
          ...
          lingering_seats: isIncrease
            ? null
            : primarySubscription.stripeSubscription.quantity - newQuantity,
  ...
  reverseSubscriptionCancellation: async ({
    locals: { getSession, supabaseServiceRole },
  }) => {
    const session = await getSession()
    if (!session) {
    ...
    try {
      ...
      return {
        success: true,
        message:
          "Subscription cancellation reversed. Your subscription will continue.",
        cancelAtPeriodEnd: updatedSubscription.cancel_at_period_end,
      }
    } catch (e) {
      ...
      return {
        success: false,
        message:
          "Failed to reverse subscription cancellation. Please try again or contact support.",
      }
    }
  },

  getProratedSeatsPreview: async ({
    request,
    locals: { getSession, supabaseServiceRole },
  }) => {
  ...
  getIntervalChangePreview: async ({
    request,
    locals: { getSession, supabaseServiceRole },
  }) => {
    ...
    const currentAnchorDate = new Date(
      primarySubscription.stripeSubscription.current_period_end * 1000,
    )
    ...
    try {
      ...
      const currentPrice = prices.data.find(
        (price) =>
          ...
          primarySubscription.stripeSubscription.plan.interval,
      )
      ...
      if (!currentPrice || !newPrice) {
        throw error(
          400,
          "Unable to find valid prices for the current or new billing cycle",
        )
      }
  ...
  changeInterval: async ({
    ...
    const newPriceId =
      primarySubscription.appSubscription.stripe_price_id[newInterval]
