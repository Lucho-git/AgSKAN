...
<script lang="ts">
  ...
  async function loadDuplicates() {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("find_duplicate_fields", {
        p_map_id: mapId,
      })
  ...
  function keepAllLeft() {
    duplicates.forEach((d) => (d.keepA = true))
    duplicates = [...duplicates]
  }
  function keepAllRight() {
    duplicates.forEach((d) => (d.keepA = false))
    duplicates = [...duplicates]
  }
  ...
  async function removeAllUnselected() {
    if (
      !confirm(
        `Remove ${duplicates.length} unselected field${duplicates.length !== 1 ? "s" : ""}? Selected fields will be kept.`,
      )
    )
      return
    resolving = true
    ...
    for (const d of duplicates) {
      ...
      try {
        const { error } = await supabase.rpc("delete_duplicate_field", {
          p_delete_field_id: deleteId,
          p_keep_field_id: keepId,
        })
        if (!error) done++
      } catch (_) {}
    }
...
<div class="dup-cleaner">
  <button class="dup-toggle" on:click={() => (expanded = !expanded)}>
    ...
    {#if duplicates.length > 0}<span class="dup-badge">{duplicates.length}</span
      >{/if}
  </button>
  {#if expanded}
    <div class="dup-panel">
      {#if loading}
        <div class="dup-loading">
          <Loader2 size={20} class="animate-spin text-blue-400" /> Scanning fields...
        </div>
      {:else if duplicates.length === 0}
        <div class="dup-empty">
          <Check size={20} class="text-green-400" />
          <span>No duplicate fields found</span>
        </div>
      {:else}
        <div class="dup-toolbar">
          <AlertCircle size={14} class="text-amber-400" />
          <span
            >{duplicates.length} pair{duplicates.length !== 1 ? "s" : ""}</span
          >
          <span class="dup-hint">— Click a field to select it for keeping</span>
          <button class="dup-side-btn" on:click={keepAllLeft}
            >Keep all left</button
          >
          <button class="dup-side-btn" on:click={keepAllRight}
            >Keep all right</button
          >
        </div>
        <div class="dup-list">
          {#each duplicates as dup, i}
            <div class="dup-item">
              <button
                class="dup-field"
                class:selected={dup.keepA}
                on:click={() => toggleKeep(i)}
              >
                <SprayRecordThumbnail
                  fieldBoundary={null}
                  record={{ id: `dup-${dup.field_a_id}` }}
                  width={80}
                  height={60}
                />
                <div class="dup-field-info">
                  <span class="dup-field-name">{dup.field_a_name}</span>
                  {#if dup.field_a_farm}<span class="dup-field-farm"
                      >{dup.field_a_farm}</span
                    >{/if}
                  <span class="dup-field-area">{dup.field_a_area_ha} ha</span>
                  {#if dup.keepA}<span class="dup-keep-badge">Keeping</span
                    >{/if}
                </div>
              </button>
              ...
              >
                <SprayRecordThumbnail
                  fieldBoundary={null}
                  record={{ id: `dup-${dup.field_b_id}` }}
                  width={80}
                  height={60}
                />
                <div class="dup-field-info">
                  <span class="dup-field-name">{dup.field_b_name}</span>
                  {#if dup.field_b_farm}<span class="dup-field-farm"
                      >{dup.field_b_farm}</span
                    >{/if}
        ...
        <div class="dup-footer">
          <button
            class="dup-remove-btn"
            on:click={removeAllUnselected}
            disabled={resolving || duplicates.length === 0}
          >
            {#if resolving}<Loader2
                size={16}
                class="animate-spin"
...
<style>
  ...
  .dup-remove-btn:hover:not(:disabled) {
    background: rgba(239, 68, 68, 0.25);
  }
  .dup-remove-btn:disabled {
    opacity: 0.4;
    cursor: not-allowed;
  }
</style>
