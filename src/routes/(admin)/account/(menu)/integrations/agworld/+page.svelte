<script lang="ts">
  ...
  const QUICK_QUERIES = [
    ...
    {
      label: "Search Crop Varieties",
      endpoint: "search-crops",
      needsGrower: false,
    },
  ]

  $: selectedAccount = accounts.find((a) => a.id === selectedAccountId) ?? null
  $: instanceLabel = selectedAccount
    ? (AGWORLD_INSTANCES[selectedAccount.instance]?.label ?? "US")
    : ""
    ...
    try {
      ...
      if (oldRaw) {
        ...
        if (Array.isArray(oldAccounts) && oldAccounts.length > 0) {
          ...
          accounts = await agworldAccountStore.getAll()
          console.log(
            `[Agworld] Migrated ${oldAccounts.length} accounts from localStorage to Supabase`,
          )
        }
      }
    } catch {}
  ...
  async function handleAddAccount() {
    ...
    adding = true
    try {
      console.log("[Agworld] Adding account:", {
        name: newName.trim(),
        instance: newInstance,
        growerId: newGrowerId.trim(),
      })
      const result = await agworldAccountStore.add(
        newName.trim(),
        newApiKey.trim(),
        newInstance,
        newGrowerId.trim(),
  ...
  async function selectAccount(id: string) {
    ...
    requestLogs = []
    expandedLogId = null
    expandedFarms = new Set()
    expandedFields = new Set()
    expandedActivityFields = new Set()
  ...
  async function linkAccountToProfile(profile: {
    email: string
    full_name: string
    master_map_id: string | null
  }) {
  ...
  async function runQuery(endpoint: string) {
    ...
    try {
      ...
      switch (endpoint) {
        case "growers":
          result = await agworldApiV3.listGrowers(a, {
            page: { size: 10 },
        ...
        case "farms":
          if (!queryGrowerId) {
            apiError = "Grower ID required"
            apiLoading = false
            return
          }
          result = await agworldApiV3.listFarms(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "fields":
          ...
          result = await agworldApiV3.listFields(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "field-boundaries":
          ...
          result = await agworldApiV3.listFieldBoundaries(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "field-crops":
          ...
          result = await agworldApiV3.listFieldCrops(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "actuals":
          ...
          result = await agworldApiV3.listActuals(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "observations":
          ...
          result = await agworldApiV3.listObservations(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "plans":
          ...
          result = await agworldApiV3.listPlans(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "recommendations":
          ...
          result = await agworldApiV3.listRecommendations(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "work-orders":
          ...
          result = await agworldApiV3.listWorkOrders(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "activity-ops":
          ...
          result = await agworldApiV3.listActivityOperations(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "product-apps":
          ...
          result = await agworldApiV3.listActivityProductApplications(
            a,
            queryGrowerId,
            { page: { size: 10 } },
          )
        ...
        case "seedings":
          ...
          result = await agworldApiV3.listActivitySeedings(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "weather":
          ...
          result = await agworldApiV3.listActivityWeatherRecords(
            a,
            queryGrowerId,
            { page: { size: 10 } },
          )
        ...
        case "activity-assets":
          ...
          result = await agworldApiV3.listActivityAssets(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "activity-problems":
          ...
          result = await agworldApiV3.listActivityProblems(a, queryGrowerId, {
            page: { size: 10 },
          })
          break
        case "operators":
        ...
        case "search-products":
          result = await agworldApiV3.searchProducts(
            a,
            queryGrowerId || "glyphosate",
            { page: { size: 10 } },
          )
          break
        case "search-crops":
          result = await agworldApiV3.searchCropVarieties(
            a,
            queryGrowerId || "wheat",
          )
          break
  ...
  function copyLog(log: AgworldRequestLog) {
    const text = [
      ...
      `Response Headers:`,
      ...(log.responseHeaders
        ? Object.entries(log.responseHeaders).map(([k, v]) => `  ${k}: ${v}`)
        : ["  (none)"]),
      ``,
      `Response Body:`,
  ...
  function saveConsolidationMetadata() {
    ...
    toast.success(
      `Consolidation package saved (${consolidatedFields.length} fields)`,
    )
  }
  ...
  // agworld_records only stores operation_id — resolve display names from the
  // `operations` table (the FieldTrailOverlay dropdown reads operation_name).
  let operationNameMap = new Map<string, string>()
  async function loadOperationNames() {
    const acct = selectedAccount
    if (!acct?.linkedMapId) return
    try {
      const { data } = await supabase
        .from("operations")
        .select("id, name")
        .eq("master_map_id", acct.linkedMapId)
      operationNameMap = new Map((data || []).map((o: any) => [o.id, o.name]))
    } catch {}
  }
  function enrichRecordOperations(records: any[]): any[] {
    for (const r of records) {
      if (r.operation_id && !r.operation_name) {
        r.operation_name =
          operationNameMap.get(r.operation_id) || "Unknown"
      }
    }
    return records
  }
  ...
  let fieldActivityData = new Map<
    string,
    { actuals: any[]; workOrders: any[]; plans: any[]; error?: string }
  >()
  ...
  let activityModalData: {
    operations: any[]
    products: any[]
    productDetails: any[]
    seedings: any[]
    seedingVarieties: any[]
    operatorUsers: any[]
    assets: any[]
    weatherRecords: any[]
    areaHa: number | null
    loading: boolean
    error?: string
    activityAttrs?: Record<string, any>
    parentActivity?: any
    nutritionTotal?: Array<{ name: string; value: number }>
    _debug?: any
    farmName?: string | null
    fieldNames?: string[]
    fieldCropName?: string | null
    fieldBoundary?: any
  } | null = null
  let activityDialogEl: HTMLDialogElement
  let activityLayout: "A" | "B" | "C" = "A"
  let activitySubLayout = 1
  function setActivitySubLayout(n: number) {
    activitySubLayout = n
  }

  function setActivityLayout(opt: string) {
    activityLayout = opt as "A" | "B" | "C"
  }

  $: if (activityModal && activityDialogEl) {
    if (!activityDialogEl.open) activityDialogEl.showModal()
  } else if (!activityModal && activityDialogEl?.open) {
  ...
  $: farmGroups = (() => {
    const groups: Record<
      string,
      { name: string; fields: any[]; totalHa: number }
    > = {}
  ...
  function toggleFarmExpand(name: string) {
    ...
    if (expanding) {
      ...
      const farmFields = agworldFieldList.filter(
        (f) => (f.farm_name || "No Farm") === name,
      )
  ...
  async function openActivityModal(activityId: string, resource: any) {
    ...
    console.log(
      `[Modal] Account check — name: ${selectedAccount.name}, hasKey: ${!!selectedAccount.apiKey}, instance: ${selectedAccount.instance}, growerId: ${selectedAccount.growerId}`,
    )
    const type =
      resource.type === "actuals"
        ? "Actual"
        : resource.type === "work-orders"
          ? "Work Order"
          : "Plan"
    ...
    console.log(
      `[Modal] Opening for ${type} "${resource.attributes?.name}" (${activityId})`,
    )
    try {
      // Fetch full individual resource for rich attributes
      let fullResource = resource
      try {
        const resPath =
          resource.type === "actuals"
            ? `/api/v3/growers/${selectedAccount.growerId}/actuals/${activityId}`
            : resource.type === "plans"
              ? `/api/v3/growers/${selectedAccount.growerId}/plans/${activityId}`
              : resource.type === "work-orders"
                ? `/api/v3/growers/${selectedAccount.growerId}/work-orders/${activityId}`
                : null
        if (resPath) {
          const r = await fetch(
            `/agworld-v3-proxy/${selectedAccount.instance}${resPath}`,
            {
              headers: {
                Authorization: `API-Key ${selectedAccount.apiKey}`,
          ...
          if (r.ok) {
            ...
            if (j.data) fullResource = j.data
          }
        }
      } catch (e) {
        console.warn("[Modal] Full resource fetch failed:", e)
      }

      const [opsRes, prodsRes, usersRes, assetsRes, weatherRes, fieldsRes] =
        await Promise.all([
          agworldApiV3.listActivityOperations(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId }, page: { size: 10 } },
          ),
          agworldApiV3.listActivityProductApplications(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId }, page: { size: 20 } },
          ),
          agworldApiV3.listActivityOperatorUsers(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId } },
          ),
          agworldApiV3.listActivityAssets(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId } },
          ),
          agworldApiV3.listActivityWeatherRecords(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId } },
          ),
          agworldApiV3.listActivityFields(
            selectedAccount,
            selectedAccount.growerId,
            { filter: { activity_id: activityId } },
          ),
      ...
      const errors: string[] = []
      if (!opsRes.success) errors.push(`ops:${opsRes.status}`)
      if (!prodsRes.success) errors.push(`prods:${prodsRes.status}`)
      if (!usersRes.success) errors.push(`users:${usersRes.status}`)
      if (!assetsRes.success) errors.push(`assets:${assetsRes.status}`)
      if (!weatherRes.success) errors.push(`weather:${weatherRes.status}`)
      if (!fieldsRes.success) errors.push(`fields:${fieldsRes.status}`)
      ...
      // Enrich products with individual detail (rate, volume, weight)
      let enrichedProducts = products
      if (products.length > 0) {
        enrichedProducts = await Promise.all(
          products.map(async (p: any) => {
            try {
              const url = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/activity-product-applications/${p.id}`
      ...
      // Enrich operations with individual detail
      let enrichedOps = ops
      if (ops.length > 0) {
        enrichedOps = await Promise.all(
          ops.map(async (o: any) => {
            try {
              const url = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/activity-operations/${o.id}`
              const r = await fetch(url, {
                headers: {
                  Authorization: `API-Key ${selectedAccount!.apiKey}`,
                  Accept: "application/vnd.api+json",
      ...
      let seedings: any[] = [],
        seedingVarieties: any[] = [],
        seedingsOk = false,
        seedingsStatus: number | undefined
      try {
        const sRes = await agworldApiV3.listActivitySeedings(
          selectedAccount,
          selectedAccount.growerId,
          { filter: { activity_id: activityId }, page: { size: 20 } },
        )
        seedingsOk = sRes.success
        seedingsStatus = sRes.status
        if (sRes.success) {
          const sList = sRes.data?.data || []
          seedings = await Promise.all(
            sList.map(async (s: any) => {
              try {
                const url = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/activity-seedings/${s.id}`
                const r = await fetch(url, {
                  headers: {
                    Authorization: `API-Key ${selectedAccount!.apiKey}`,
                    Accept: "application/vnd.api+json",
          ...
          seedingVarieties = (
            await Promise.all(
              seedings.map(async (s: any) => {
                const varId = s.relationships?.crop_variety?.data?.id
                if (!varId) return null
                try {
                  const url = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/crop-varieties/${varId}`
                  const r = await fetch(url, {
                    headers: {
                      Authorization: `API-Key ${selectedAccount!.apiKey}`,
                      Accept: "application/vnd.api+json",
        ...
        console.warn("[Modal] Seedings fetch failed:", e)
      }

      // Fetch global product details (active ingredients, category, formulation)
      let productDetails: any[] = []
      if (enrichedProducts.length > 0) {
        productDetails = (
          await Promise.all(
            enrichedProducts.map(async (p: any) => {
              const prodId = p.relationships?.product?.data?.id
              if (!prodId) return null
              try {
                const url = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/products/${prodId}`
                const r = await fetch(url, {
                  headers: {
                    Authorization: `API-Key ${selectedAccount!.apiKey}`,
                    Accept: "application/vnd.api+json",
      ...
      // Fetch field details (name, farm, crop)
      let fieldNames: string[] = [],
        farmName: string | null = null,
        fieldCropName: string | null = null,
        areaHa: number | null = null,
        fieldBoundary: any = null
      if (fields.length > 0) {
        fieldNames = fields.map((f: any) => f.attributes?.name || "Unknown")
        const fb = fields[0]
        const boundaryRef = fb.relationships?.field_boundary?.data
        if (boundaryRef?.id) {
          try {
            const bUrl = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/field-boundaries/${boundaryRef.id}`
            ...
            if (bR.ok) {
              ...
              if (bJ.data?.attributes?.area?.scalar != null)
                areaHa = bJ.data.attributes.area.scalar
              if (bJ.data?.attributes?.boundary)
                fieldBoundary = bJ.data.attributes.boundary
              const fieldRef = bJ.data?.relationships?.field?.data
              if (fieldRef?.id) {
                try {
                  const fUrl = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/fields/${fieldRef.id}`
                  ...
                  if (fR.ok) {
                    ...
                    const farmRef = fJ.data?.relationships?.farm?.data
                    if (farmRef?.id) {
                      try {
                        const farmUrl = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/farms/${farmRef.id}`
                        const farmR = await fetch(farmUrl, {
                          headers: {
                            Authorization: `API-Key ${selectedAccount!.apiKey}`,
                            Accept: "application/vnd.api+json",
                          },
                        ...
                        if (farmR.ok) {
                          const farmJ = await farmR.json()
                          farmName = farmJ.data?.attributes?.name || null
                        }
                      } catch {}
                    }
                    const fcRefs = fJ.data?.relationships?.field_crops?.data
                    if (fcRefs?.length > 0) {
                      try {
                        const fcUrl = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/growers/${selectedAccount!.growerId}/field-crops/${fcRefs[0].id}`
                        ...
                        if (fcR.ok) {
                          ...
                          const cvRef =
                            fcJ.data?.relationships?.crop_variety?.data
                          if (cvRef?.id) {
                            const cvUrl = `/agworld-v3-proxy/${selectedAccount!.instance}/api/v3/crop-varieties/${cvRef.id}`
                            const cvR = await fetch(cvUrl, {
                              headers: {
                                Authorization: `API-Key ${selectedAccount!.apiKey}`,
                                Accept: "application/vnd.api+json",
                              },
                            ...
                            if (cvR.ok) {
                              const cvJ = await cvR.json()
                              fieldCropName =
                                (cvJ.data?.attributes?.crop_type?.name || "") +
                                " - " +
                                (cvJ.data?.attributes?.name || "")
                            }
                          }
                        }
                      } catch {}
      ...
      if (areaHa == null && fields.length > 0)
        areaHa = fields.reduce(
          (sum: number, f: any) =>
            sum + ((f.attributes?.area?.scalar || 0) as number),
          0,
        )

      const actAttrs = fullResource.attributes || {}

      // Fetch parent activity
      let parentActivity: any = null
      const parentRef = fullResource.relationships?.parent_activity?.data
      if (parentRef?.id) {
        try {
          const pPath =
            parentRef.type === "plans"
              ? `/api/v3/growers/${selectedAccount.growerId}/plans/${parentRef.id}`
              : parentRef.type === "work-orders"
                ? `/api/v3/growers/${selectedAccount.growerId}/work-orders/${parentRef.id}`
                : `/api/v3/${parentRef.type}/${parentRef.id}`
          const pR = await fetch(
            `/agworld-v3-proxy/${selectedAccount.instance}${pPath}`,
            {
              headers: {
                Authorization: `API-Key ${selectedAccount.apiKey}`,
                Accept: "application/vnd.api+json",
          ...
          if (pR.ok) {
            ...
            parentActivity = pJ.data
          }
        } catch {}
      }

      // Compute nutrition (kg/ha)
      const waterRate = actAttrs.total_application_rate?.scalar || 0
      const nutritionMap = new Map<string, number>()
      for (const prod of enrichedProducts) {
        const rate = prod.attributes?.application_rate
        if (!rate?.scalar) continue
        let ratePerHa = rate.scalar
        if (rate.units === "g/ha") ratePerHa = rate.scalar / 1000
        else if (
          prod.attributes?.mix_method === "per_volume" &&
          waterRate > 0 &&
          rate.units === "L/100L"
        )
          ratePerHa = rate.scalar * (waterRate / 100)
        const detail = productDetails.find(
          (pd: any) => pd.id === prod.relationships?.product?.data?.id,
        )
        for (const ing of detail?.attributes?.active_ingredients || []) {
          const key = ing.name as string
          const pct = (ing.content?.scalar || 0) / 100
          nutritionMap.set(key, (nutritionMap.get(key) || 0) + ratePerHa * pct)
        }
      }
      const nutritionTotal = [...nutritionMap.entries()]
        .map(([name, value]) => ({
          name,
      ...
      activityModalData = {
        ...
        _debug: {
          fullResourceFetched: fullResource !== resource,
          fullResourceKeys: Object.keys(fullResource.attributes || {}),
          listResourceKeys: Object.keys(resource.attributes || {}),
          parentFetched: !!parentActivity,
          parentRef: resource.relationships?.parent_activity?.data || null,
          apiResults: {
            operations: {
              success: opsRes.success,
              status: opsRes.status,
              count: enrichedOps.length,
            },
            products: {
              success: prodsRes.success,
              status: prodsRes.status,
            ...
            fields: {
              success: fieldsRes.success,
              status: fieldsRes.status,
              count: fields.length,
            },
            seedings: {
              success: seedingsOk,
              status: seedingsStatus,
              count: seedings.length,
              varieties: seedingVarieties.length,
            },
            productDetails: {
              success:
                productDetails.length > 0 || enrichedProducts.length === 0,
  ...
  /** Format an Agworld attribute value — handles {scalar, units} objects */
  function fmtAttr(v: any): string {
    if (v == null) return "-"
    if (typeof v === "number") return v % 1 === 0 ? String(v) : v.toFixed(2)
    if (typeof v === "object" && v.scalar != null) {
      const s =
        Number(v.scalar) % 1 === 0
          ? String(v.scalar)
          : Number(v.scalar).toFixed(2)
      return v.units ? `${s} ${v.units}` : s
    }
  ...
  async function fetchRelationship(resource: any, relName: string) {
    ...
    try {
      const path =
        target.type === "growers"
          ? `/api/v3/growers/${target.id}`
          : target.type === "products"
            ? `/api/v3/products/${target.id}`
            : target.type === "operations"
              ? `/api/v3/operations/${target.id}`
              : target.type === "users"
                ? `/api/v3/users/${target.id}`
                : target.type === "actuals"
                  ? `/api/v3/growers/${selectedAccount.growerId}/actuals/${target.id}`
                  : target.type === "plans"
                    ? `/api/v3/growers/${selectedAccount.growerId}/plans/${target.id}`
                    : target.type === "work-orders"
                      ? `/api/v3/growers/${selectedAccount.growerId}/work-orders/${target.id}`
                      : target.type === "recommendations"
                        ? `/api/v3/growers/${selectedAccount.growerId}/recommendations/${target.id}`
      ...
      surfStack = [
        ...prev,
        { label, data: null, loading: false, error: e.message },
      ]
    }
  }
  ...
  function getMergedTimeline(activity: {
    actuals: any[]
    workOrders: any[]
    plans: any[]
    ...
    const getName = (r: any) =>
      (
        (r.attributes?.name ||
          r.attributes?.operation_type_name ||
          "") as string
      )
        .trim()
        .toLowerCase()
    ...
    if (convertedIds.size > 0) {
      ...
      for (const plan of plans) {
        ...
        const matchingActual = actuals.find(
          (a) => convertedIds.has(a.id) && getName(a) === planName,
        )
        if (matchingActual) {
      ...
      for (const plan of plans) {
        ...
        if (!planName) continue
        const matchingActual = actuals.find(
          (a) => getName(a) === planName && !matchedActualIds.has(a.id),
        )
        if (matchingActual) {
          convertedPairs.push({ plan, actual: matchingActual })
          matchedPlanIds.add(plan.id)
    ...
    const discardedPlans = allPending.filter(
      (p) => p.attributes?.status === "closed_discarded",
    )
    const pendingPlans = allPending.filter(
      (p) => p.attributes?.status !== "closed_discarded",
    )

    // Sort by date — oldest first
    const sortDate = (r: any) =>
      r.attributes?.completed_at ||
      r.attributes?.due_at ||
      r.attributes?.started_at ||
      ""

    return {
      convertedPairs: convertedPairs.sort((a, b) =>
        sortDate(a.actual).localeCompare(sortDate(b.actual)),
      ),
      discardedConvertedPairs: discardedConvertedPairs.sort((a, b) =>
        sortDate(a.actual).localeCompare(sortDate(b.actual)),
      ),
      unmatchedActuals: unmatchedActuals.sort((a, b) =>
        sortDate(a).localeCompare(sortDate(b)),
      ),
      pendingPlans: pendingPlans.sort((a, b) =>
        sortDate(a).localeCompare(sortDate(b)),
      ),
      discardedPlans: discardedPlans.sort((a, b) =>
        sortDate(a).localeCompare(sortDate(b)),
      ),
      workOrders: workOrders.sort((a, b) =>
        (a.attributes?.due_date || "").localeCompare(
          b.attributes?.due_date || "",
        ),
      ),
    }
  }
  ...
  async function loadAgworldActivitiesFromDb(accountId?: string) {
    ...
    try {
      ...
      for (const field of agworldFieldList) {
        const boundaryId =
          field.properties?.agworld_boundary_id || field.agworld_field_ref
        const activities = byBoundary.get(boundaryId) || []
        if (activities.length > 0) {
          newMap.set(field.id, {
            actuals: activities
              .filter((a) => a.agworld_type === "actual")
              .map((a) => a.data),
            workOrders: activities
              .filter((a) => a.agworld_type === "work_order")
              .map((a) => a.data),
            plans: activities
              .filter((a) => a.agworld_type === "plan")
              .map((a) => a.data),
          })
        }
      }
      ...
      console.log(
        `[AgActivity] Loaded ${data?.length || 0} cached activities from DB across ${newMap.size} fields`,
      )
    } catch (e: any) {
  ...
  async function saveActivitiesToDb(
    field: any,
    items: { type: string; resource: any }[],
  ) {
    ...
    const { error } = await supabase
      .from("agworld_activities")
      .upsert(rows, {
        onConflict: "account_id,agworld_id,agworld_type",
  ...
  async function fetchFieldActivity(field: any) {
    ...
    console.log(
      `[AgActivity] Fetching for field "${field.name}" (boundaryId: ${boundaryId}, fieldId: ${field.agworld_field_ref}, grower: ${selectedAccount.growerId})`,
    )
    try {
      ...
      try {
        actualsRes = await agworldApiV3.listActualsByField(
          selectedAccount,
          selectedAccount.growerId,
          boundaryId,
          { page: { size: 50 }, sort: "-completed_at" },
        )
      } catch (e: any) {
        actualsRes = {
          success: false,
      ...
      try {
        workOrdersRes = await agworldApiV3.listWorkOrdersByField(
          selectedAccount,
          selectedAccount.growerId,
          boundaryId,
          { page: { size: 50 }, sort: "-updated_at" },
        )
      } catch (e: any) {
        workOrdersRes = {
          success: false,
      ...
      try {
        plansRes = await agworldApiV3.listPlansByField(
          selectedAccount,
          selectedAccount.growerId,
          boundaryId,
          { page: { size: 50 }, sort: "-updated_at" },
      ...
      console.log(
        `  actuals:    success=${actualsRes.success} status=${actualsRes.status} count=${actualsRes.data?.data?.length ?? 0} error=${actualsRes.error || "none"}`,
      )
      console.log(
        `  workOrders: success=${workOrdersRes.success} status=${workOrdersRes.status} count=${workOrdersRes.data?.data?.length ?? 0} error=${workOrdersRes.error || "none"}`,
      )
      console.log(
        `  plans:      success=${plansRes.success} status=${plansRes.status} count=${plansRes.data?.data?.length ?? 0} error=${plansRes.error || "none"}`,
      )
      console.log(`  duration: ${elapsed}ms`)

      // Log response bodies if any call failed
      if (!actualsRes.success)
        console.warn(`[AgActivity] Actuals FAILED — response:`, actualsRes)
      if (!workOrdersRes.success)
        console.warn(
          `[AgActivity] WorkOrders FAILED — response:`,
          workOrdersRes,
        )
      if (!plansRes.success)
        console.warn(`[AgActivity] Plans FAILED — response:`, plansRes)
      ...
      if (convertedPlanIds.length > 0) {
        try {
          ...
          const parentRes = await agworldApiV3.listActualsByField(
            selectedAccount,
            selectedAccount.growerId,
            boundaryId,
            {
          ...
          console.log(
            `[AgActivity] "${field.name}" parent_id query — ${convertedActuals.length} converted actuals from ${convertedPlanIds.length} plans`,
          )
          if (!parentRes.success)
            console.warn(
              `[AgActivity] parent_id query FAILED:`,
              parentRes.error,
            )
        } catch (e: any) {
      ...
      const standaloneActuals = actuals.filter(
        (a: any) => !convertedActualIds.has(a.id),
      )

      const errors: string[] = []
      if (!actualsRes.success) errors.push(`Actuals: ${actualsRes.error}`)
      if (!workOrdersRes.success)
        errors.push(`Work Orders: ${workOrdersRes.error}`)
      ...
      console.error(
        `[AgActivity] "${field.name}" fetch exception:`,
        e.message || e,
      )
  ...
  async function syncAllActivity() {
    const fields = agworldFieldList.filter(
      (f) => f.properties?.agworld_boundary_id || f.agworld_field_ref,
    )
    if (fields.length === 0) {
    ...
    for (let i = 0; i < fields.length; i += BATCH) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (f) => {
          if (!selectedAccount || !selectedAccount.growerId) return
          const boundaryId =
            f.properties?.agworld_boundary_id || f.agworld_field_ref
          ...
          try {
            ...
            try {
              ...
              workOrdersRes = { success: false, error: e.message, data: null }
            }
            ...
            try {
              ...
              plansRes = { success: false, error: e.message, data: null }
            }
            ...
            if (convertedPlanIds.length > 0) {
              try {
                ...
                convertedActualIds = new Set(
                  convertedActuals.map((a: any) => a.id),
                )
              } catch {
                /* non-critical */
              }
    ...
    if (failed > 0) {
      toast.warning(
        `Synced ${totalItems} items across ${synced} fields · ${failed} failed`,
      )
    } else {
      toast.success(
        `Synced ${totalItems} activity items across ${synced} fields`,
      )
    }
  }
  ...
  async function loadAgworldFieldList(accountId?: string) {
    ...
    try {
      const { data } = await supabase
        .from("agworld_fields")
        .select(
          "id, name, agworld_field_name, agworld_field_ref, area, boundary, farm_id, farm_name, crop_type, production_year, properties, last_activity_sync_at",
        )
        .eq("account_id", acctId)
        .order("name")
  ...
  async function openFieldOverlay(field: any) {
    if (!field?.boundary) return
    overlayField = field
    overlayInputProducts = []
    overlayCrop = ""
    const acct = selectedAccount
    if (operationNameMap.size === 0 && acct?.linkedMapId)
      await loadOperationNames()
    try {
      // Load full records for this field (with geometry) on demand — the list
      // only keeps light columns, so older history shows here.
      const { data } = acct
        ? await supabase
            .from("agworld_records")
            .select("*")
            .eq("account_id", acct.id)
            .eq("agworld_field_id", field.id)
            .order("start_time", { ascending: true })
        : { data: null }
      overlayRecords = data?.length
        ? enrichRecordOperations(data)
        : enrichRecordOperations(
            agworldRecords.filter((r) => r.agworld_field_id === field.id),
          )
    } catch {
      overlayRecords = enrichRecordOperations(
        agworldRecords.filter((r) => r.agworld_field_id === field.id),
      )
    }
    gatherFieldInputs(field)
    // Ensure the Agworld activity cache is loaded so the overlay can suggest
    // plans/actuals against this field's trails (predictive, nothing linked).
    if (!fieldActivityData.has(field.id)) loadAgworldActivitiesFromDb(selectedAccount?.id)
    showMap = true
  }

  // Pull real Agworld inputs (product applications + seedings) for this field's activities
  async function gatherFieldInputs(field: any) {
    const acct = selectedAccount
    if (!acct?.growerId) return
    overlayCrop = field.crop_type || field.cropType || ""
    const cached = fieldActivityData.get(field.id)
    const items = [
      ...(cached?.actuals || []),
      ...(cached?.workOrders || []),
      ...(cached?.plans || []),
    ...
    // Collect product applications + seedings across the field's activities
    const apps: any[] = []
    const seedings: any[] = []
    for (const item of items) {
      try {
        const res = await agworldApiV3.listActivityProductApplications(
          acct,
          acct.growerId,
          { filter: { activity_id: item.id }, page: { size: 50 } },
        )
        if (res.success) apps.push(...(res.data?.data || []))
      } catch {}
      try {
        const sRes = await agworldApiV3.listActivitySeedings(
          acct,
          acct.growerId,
          { filter: { activity_id: item.id }, page: { size: 20 } },
        )
        if (sRes.success) seedings.push(...(sRes.data?.data || []))
      } catch {}
    }
    if (apps.length === 0 && seedings.length === 0) return

    const getJson = async (url: string) => {
      try {
        const r = await fetch(url, {
          headers: {
            Authorization: `API-Key ${acct.apiKey}`,
            Accept: "application/vnd.api+json",
          },
        })
    ...
    // Enrich product application details (rate + total qty live on the detail endpoint)
    const appDetail = new Map<string, any>()
    await Promise.all(
      apps.slice(0, 120).map(async (a: any) => {
        const j = await getJson(
          `/agworld-v3-proxy/${acct.instance}/api/v3/growers/${acct.growerId}/activity-product-applications/${a.id}`,
        )
        if (j?.data) appDetail.set(a.id, j.data)
      }),
    )
    const appAttrs = (a: any) =>
      appDetail.get(a.id)?.attributes || a.attributes || {}

    // Global product details (active ingredients) for distinct products
    const prodIds = [
      ...new Set(
        apps.map((a) => a.relationships?.product?.data?.id).filter(Boolean),
      ),
    ].slice(0, 50)
    const prodDetails = new Map<string, any>()
    await Promise.all(
      prodIds.map(async (pid: string) => {
        const j = await getJson(
          `/agworld-v3-proxy/${acct.instance}/api/v3/products/${pid}`,
        )
        if (j?.data) prodDetails.set(pid, j.data)
      }),
    )

    // Seed detail + crop variety names
    const seedName = new Map<string, string>() // seeding id -> variety name
    await Promise.all(
      seedings.slice(0, 60).map(async (s: any) => {
        const j = await getJson(
          `/agworld-v3-proxy/${acct.instance}/api/v3/growers/${acct.growerId}/activity-seedings/${s.id}`,
        )
        const d = j?.data
        if (!d) return
        const varId = d.relationships?.crop_variety?.data?.id
        let vname = ""
        if (varId) {
          const vj = await getJson(
            `/agworld-v3-proxy/${acct.instance}/api/v3/crop-varieties/${varId}`,
          )
          vname = vj?.data?.attributes?.name || ""
        }
        seedName.set(s.id, vname || d.attributes?.crop_name || "")
      }),
    )

    // Aggregate into report rows (products + seeds)
    ...
    const addRow = (
      ...
      if (e) {
        ...
        if ((!e.rate || e.rate === "—") && rate) e.rate = rate
        if ((!e.activeIngredient || e.activeIngredient === "—") && ai)
          e.activeIngredient = ai
      } else {
        byName.set(name, {
          name,
          activeIngredient: ai || "—",
    ...
    for (const a of apps) {
      const attrs = appAttrs(a)
      const name = String(attrs.name || "Unknown Product")
      const pid = a.relationships?.product?.data?.id
      const detail = pid ? prodDetails.get(pid) : null
      const ais = (detail?.attributes?.active_ingredients || [])
        .map((i: any) => i.name)
        .filter(Boolean)
        .join(", ")
      const rate = attrs.application_rate
      const rateStr =
        rate?.scalar != null
          ? `${rate.scalar}${rate.units ? " " + rate.units : ""}`
          : ""
      const qty =
        parseFloat(attrs.amount) ||
        parseFloat(attrs.volume) ||
        parseFloat(attrs.weight) ||
        0
      addRow(name, ais, rateStr, qty, attrs.units || rate?.units || "")
    }
    for (const s of seedings) {
      const attrs = s.attributes || {}
      const vname = seedName.get(s.id) || ""
      const name = vname || attrs.crop_name || "Seed"
      const rate = attrs.seeding_rate
      const rateStr =
        rate?.scalar != null
          ? `${rate.scalar}${rate.units ? " " + rate.units : ""}`
          : ""
      const qty =
        parseFloat(attrs.amount) ||
        parseFloat(attrs.seed_amount) ||
        parseFloat(attrs.weight) ||
        0
      addRow(name, "Seed", rateStr, qty, attrs.units || rate?.units || "")
    }

    const out = [...byName.values()].map((p: any) => ({
      name: p.name,
      activeIngredient: p.activeIngredient,
      rate: p.rate,
      usage:
        p.usage > 0
          ? `${Math.round(p.usage * 1000) / 1000}${p.unit ? " " + p.unit : ""}`.trim()
          : "—",
    }))
    if (out.length) {
      overlayInputProducts = out
      overlayCrop = field.crop_type || field.cropType || ""
    }
  }
  ...
  async function loadAgworldRecords(accountId?: string) {
    ...
    await loadOperationNames()
    try {
      // Load ALL records (paginated) — geometry columns excluded; the map
      // overlay fetches full records (with geometry) on demand per field.
      const PAGE = 1000
      const all: any[] = []
      for (let page = 0; page < 20; page++) {
        const { data, error } = await supabase
          .from("agworld_records")
          .select(
            "id, account_id, agworld_field_id, agworld_field_name, start_time, end_time, duration_seconds, operator_name, operator_id, area_hectares, distance_km, vehicle_type, vehicle_marker, swath_width, trail_id, operation_id, field_id, gen_method, gen_pct_of_field, gen_pct_of_trail, gen_pct_of_dominant, gen_dominant_field_id, gen_gap_merges",
          )
          .eq("account_id", acctId)
      ...
      agworldRecords = enrichRecordOperations(all)
    } catch (e: any) {
      recordsError = e.message
    } finally {
      recordsLoading = false
  ...
  async function importAllAgworldFields() {
    ...
    try {
      ...
      while (true) {
        const fbResult = await agworldApiV3.listFieldBoundaries(
          selectedAccount,
          selectedAccount.growerId,
          { page: { number: page, size: PAGE } },
        )
        const refs = fbResult.data?.data || []
        allBoundaryRefs.push(...refs)
        if (!fbResult.success)
          pageErrors.push(
            `Boundaries p${page}: ${fbResult.status} ${fbResult.error || ""}`,
          )
      ...
      page = 1
      while (true) {
        const fdResult = await agworldApiV3.listFields(
          selectedAccount,
          selectedAccount.growerId,
          { page: { number: page, size: PAGE } },
        )
        ...
        if (!fdResult.success)
          pageErrors.push(
            `Fields p${page}: ${fdResult.status} ${fdResult.error || ""}`,
          )
        if (refs.length < PAGE) break
        page++
        await new Promise((r) => setTimeout(r, 200))
      ...
      console.log(
        `PAGINATION: ${allBoundaryRefs.length} boundaries, ${allFields.length} fields, ${allCrops.length} crops${pageErrors.length ? " — ERRORS: " + pageErrors.join("; ") : ""}`,
      )

      // Farms & crops — paginate crops too
      const fwResult = await agworldApiV3.listFarms(
        selectedAccount,
        selectedAccount.growerId,
        { page: { size: 50 } },
      )
      ...
      page = 1
      while (true) {
        const fcResult = await agworldApiV3.listFieldCrops(
          selectedAccount,
          selectedAccount.growerId,
          { page: { number: page, size: PAGE } },
        )
        ...
        if (!fcResult.success)
          pageErrors.push(
            `Crops p${page}: ${fcResult.status} ${fcResult.error || ""}`,
          )
        if (refs.length < PAGE) break
        page++
        await new Promise((r) => setTimeout(r, 200))
      ...
      const varietyIds = [
        ...new Set(
          crops
            .map((c) => c.relationships?.crop_variety?.data?.id)
            .filter(Boolean),
        ),
      ] as string[]
      ...
      // Filter to 2026 only
      const refs2026 = boundaryRefs.filter(
        (b) => b.attributes?.production_year === 2026,
      )
      ...
      toast.info(
        `Fetching geometry for ${refs2026.length} boundaries (${awFields.length} fields, 2026)...`,
      )

      // Step 2: Fetch geometry — keep one boundary per field (2026 only)
      const sorted = [...refs2026].sort((a, b) => {
        const yDiff =
          (b.attributes?.production_year || 0) -
          (a.attributes?.production_year || 0)
        if (yDiff !== 0) return yDiff
        return (b.attributes?.updated_at || "").localeCompare(
      ...
      toast.info(
        `Fetching geometry for ${toFetch.length} fields (${boundaryRefs.length} boundaries total)...`,
      )
      ...
      for (let i = 0; i < toFetch.length; i += 4) {
        ...
        for (let j = 0; j < results.length; j++) {
          ...
          if (!res.success) {
            ...
            if (errorSamples.length < 3) {
              errorSamples.push({
                boundaryId: batch[j].id,
                status: res.status,
                error: res.error,
                fieldId: batch[j].relationships?.field?.data?.id,
              })
            }
            continue
          }
          ...
          const fieldId =
            d.relationships?.field?.data?.id ||
            batch[j].relationships?.field?.data?.id
          const name =
            fieldNameMap.get(fieldId) ||
            attrs.name ||
            `Field ${batch[j].id?.substring(0, 8)}`
          ...
          const prodYear =
            batch[j].attributes?.production_year ||
            attrs.production_year ||
            null
          ...
          const { error } = await supabase.from("agworld_fields").upsert(
            ...
            {
              onConflict: "account_id,agworld_field_ref,production_year",
              ignoreDuplicates: false,
            },
          )
          if (!error) imported++
        ...
        if (i + 4 < toFetch.length) {
          await new Promise((r) =>
            setTimeout(r, batchHad429 ? batchDelay * 2 : batchDelay),
          )
        }
      }
      ...
      if (failed > 0) {
        console.log(
          `GEOMETRY FETCH ERRORS: ${failed} failed, ${skipped} no geometry`,
          { samples: errorSamples, counts: errorCounts },
        )
      }
      toast.success(
        `Imported ${imported} fields${failed ? ` (${failed} failed, ${skipped} no geometry — see console)` : ""}`,
      )
  ...
  async function matchSprayToAgworld() {
    ...
    recordsError = null
    console.log(
      "[Match] Starting spray→agworld match, map:",
      selectedAccount.linkedMapId,
    )
    try {
      ...
      if (awErr) {
        recordsError = awErr.message
        console.error("[Match] Error loading agworld fields:", awErr.message)
        return
      }
      if (!awFields || awFields.length === 0) {
        console.warn("[Match] No agworld fields imported")
        toast.warning(
          "No agworld fields imported yet — click 'Import Agworld Fields' first",
        )
        return
      }
      ...
      if (srErr) {
        recordsError = srErr.message
        console.error("[Match] Error loading spray records:", srErr.message)
        return
      }
      if (!sprayRecs || sprayRecs.length === 0) {
        console.warn(
          `[Match] No spray records found for map ${selectedAccount.linkedMapId}. Generate them first via /account/records → Backfill.`,
        )
      ...
      const awPolys = awFields
        ...
        .filter(
          (f) => f.geom?.type === "Polygon" || f.geom?.type === "MultiPolygon",
        )
      ...
      for (const sr of sprayRecs) {
        ...
        for (const aw of awPolys) {
          try {
            ...
            if (turf.booleanPointInPolygon(pt.geometry.coordinates, awFeat)) {
              best = aw
              break
            }
          } catch {}
        ...
        const { error: upErr } = await supabase.from("agworld_records").upsert(
          {
            ...
            trail_id: sr.trail_id,
            operation_id: sr.operation_id,
            vehicle_id: sr.vehicle_id,
            operator_name: sr.operator_name,
            field_path: sr.field_path,
            area_hectares: sr.area_hectares,
            distance_km: sr.distance_km,
            start_time: sr.start_time,
            end_time: sr.end_time,
            duration_seconds: sr.duration_seconds,
            activity_type: sr.activity_type,
            chem_mix: sr.chem_mix,
            weather_data: sr.weather_data,
            swath_width: sr.swath_width,
            field_id: sr.field_id,
            operator_id: sr.operator_id,
            trail_width: sr.trail_width,
            trail_color: sr.trail_color,
            vehicle_type: sr.vehicle_type,
            vehicle_marker: sr.vehicle_marker,
            gen_pct_of_field: sr.gen_pct_of_field,
            gen_pct_of_trail: sr.gen_pct_of_trail,
            gen_method: "spatial_centroid",
            status: "matched",
          },
        ...
        if (!upErr) {
          ...
          console.log("MATCH UPSERT ERROR:", {
            error: upErr.message,
      ...
      console.log(
        `MATCH DONE: ${matched} matched, ${noMatch} no field, ${noGeom} no geometry`,
      )
      if (matched > 0 && matched < 3) {
        ...
        console.log(
          ...
          sample[0]
            ? {
                id: sample[0].id,
                field_path_type: sample[0].field_path?.type,
                has_boundary: !!sample[0].agworld_boundary,
              }
            : "none",
        )
      }
      toast.success(
        `Matched ${matched} of ${sprayRecs.length} spray records (${noMatch} no field, ${noGeom} no geometry)`,
      )
  ...
  let trailProgress = {
    total: 0,
    processed: 0,
    oldest_processed: null as string | null,
  }

  async function loadTrailProgress() {
    if (!selectedAccount?.linkedMapId) {
      trailProgress = { total: 0, processed: 0, oldest_processed: null }
      return
    }
    try {
  ...
  async function generateAgworldRecords() {
    ...
    console.log(
      `[GenAg] Processing up to ${genTrailLimit} trails, map: ${selectedAccount.linkedMapId}`,
    )
    let total = 0,
      generated = 0,
      skipped = 0
    ...
    try {
      ...
      console.log(
        `[GenAg] Done: ${total} trails → ${generated} records${skipped ? ` (${skipped} skipped)` : ""}`,
      )
      genProgress = `${generated} records from ${total} trails`
      toast.success(
        `Generated ${generated} records from ${total} trails${skipped ? ` · ${skipped} skipped` : ""}`,
      )
      await Promise.all([
        loadAgworldFieldCounts(),
        loadAgworldFieldList(),
        loadAgworldRecords(),
        loadTrailProgress(),
      ])
    } catch (e: any) {
  ...
  async function clearAgworldData() {
    ...
    if (
      !confirm(
        "Delete ALL agworld fields, records and activities for this account? This cannot be undone.",
      )
    )
      return
    recordsLoading = true
  ...
  async function undoConsolidationSave() {
    ...
    if (
      !confirm(
        "This will restore all original AgSKAN field names, areas, and boundaries from the data loaded at the start of this comparison. Continue?",
      )
    )
      return
    ...
    try {
      ...
      if (createdFields && createdFields.length > 0) {
        ...
        const toDelete = createdFields.filter(
          (f) => !originalIds.has(f.field_id),
        )
  ...
  let proposalDominant: { id: string; source: "agskan" | "agworld" } | null =
    null
  // The other fields involved (opposite side for match, candidates for merge/split)
  let proposalCandidates: Array<{
    id: string
    name: string
    source: "agskan" | "agworld"
    overlapPct: number
    areaHa: number
  }> = []
  ...
  $: showReviewOverlay =
    view === "compare" &&
    !compareLoading &&
    !reviewOverlayDismissed &&
    (reviewSkan.length > 0 || reviewAw.length > 0)
  ...
  function commitPreviewArea() {
    ...
    const cf = consolidatedFields.find((f) => f.id === selectedPreviewId)
    if (cf) {
      cf.area_ha = editingPreviewArea
      consolidatedFields = consolidatedFields
    }
    flashSaved()
  }
  function advanceToNextPreviewField() {
    const list = [...consolidatedFields].sort((a, b) =>
  ...
  function deletePreviewField() {
    ...
    const idx = list.findIndex((f) => f.id === selectedPreviewId)
    consolidatedFields = consolidatedFields.filter(
      (f) => f.id !== selectedPreviewId,
    )
    if (list.length > 1) {
      const next = list[(idx + 1) % list.length]
      selectedPreviewId =
        next.id === selectedPreviewId
          ? (list[(idx + 2) % list.length]?.id ?? null)
          : next.id
      const cf = consolidatedFields.find((f) => f.id === selectedPreviewId)
      if (cf) {
        editingPreviewName = cf.name
        editingPreviewArea = Math.round(cf.area_ha * 10) / 10
      } else {
  ...
  async function startCompare() {
    ...
    try {
      ...
      if (selectedAccount.linkedMapId) {
        ...
        const userId = session?.session?.user?.id
        if (!userId) {
          compareError = "Not logged in to AgSKAN"
      ...
      if (!mapId) {
        compareError =
          "No AgSKAN map connected. Link this account to a customer profile below to compare their specific map."
        compareLoading = false
        return
      }
      ...
      const { data: fields } = await supabase
        .from("fields")
        .select("field_id, name, area, farm_id, boundary")
        .eq("map_id", mapId)
        .order("name")
        .limit(100)
      agskanFields = fields || []
      ...
      if (selectedAccount.growerId) {
        const [fwResult, fdResult, fbListResult] = await Promise.all([
          agworldApiV3.listFarms(selectedAccount, selectedAccount.growerId, {
            page: { size: 50 },
          }),
          agworldApiV3.listFields(selectedAccount, selectedAccount.growerId, {
            page: { size: 100 },
          }),
          agworldApiV3.listFieldBoundaries(
            selectedAccount,
            selectedAccount.growerId,
            { page: { size: 100 } },
          ),
        ...
        if (fwResult.success) {
          agworldFarms = fwResult.data?.data || []
          if (fwResult.log) compareLogs = [...compareLogs, fwResult.log]
        }
        if (fdResult.success) {
          agworldFields = fdResult.data?.data || []
          if (fdResult.log) compareLogs = [...compareLogs, fdResult.log]
        }
        if (fbListResult.log) compareLogs = [...compareLogs, fbListResult.log]

        // Build field name lookup
        const fieldNames = new Map<string, string>()
        for (const f of agworldFields)
          fieldNames.set(f.id, f.attributes?.name || "")
        ...
        if (rateLimited > 0) {
          toast.warning(
            `${rateLimited} boundaries rate-limited. ${allBounds.filter((b) => b.boundary).length}/${allBounds.length} loaded.`,
          )
        }
        ...
        try {
          ...
          for (const b of allBounds) {
            if (b.boundary?.type) {
              try {
                b._turfAreaHa = turf.area(turf.feature(b.boundary)) / 10000
              } catch {
                b._turfAreaHa = null
              }
            }
  ...
  async function categorizeFields() {
    ...
    for (const skan of agskanFields) {
      ...
      for (const awB of agworldBoundaries) {
        ...
        const nameScore = findMatchScore(
          skan.name,
          awField?.attributes?.name || "",
        )

        allPairs.push({
          skanId: skan.field_id,
          awId: awB.field_id,
          iou,
          nameScore,
        })
    ...
    for (const skan of agskanFields) {
      ...
      const hasOverlap = allPairs.some(
        (p) => p.skanId === skan.field_id && p.iou > 0.01,
      )
      if (hasOverlap) {
    ...
    for (const awB of agworldBoundaries) {
      ...
      const hasOverlap = allPairs.some(
        (p) => p.awId === awB.field_id && p.iou > 0.01,
      )
      if (hasOverlap) {
        const awField = agworldFields.find((f) => f.id === awB.field_id)
  ...
  async function findConnectedGroup(id: string, source: "agskan" | "agworld") {
    ...
    const oppositeFields =
      source === "agskan" ? agworldBoundaries : agskanFields
    ...
    for (const oppId of directOverlaps) {
      ...
      if (!oppGeom) continue
      const sameSideFields =
        source === "agskan" ? agskanFields : agworldBoundaries
  ...
  async function openInspector(id: string, source: "agskan" | "agworld") {
    ...
    const buildSkanCand = (sid: string) => {
      ...
      return {
        id: sid,
        name: skan?.name || sid,
        source: "agskan" as const,
        overlapPct: 0,
        areaHa: skan?._turfAreaHa || 0,
      }
    }
    const buildAwCand = (aid: string) => {
      ...
      return {
        id: aid,
        name: awField?.attributes?.name || aid,
        source: "agworld" as const,
        overlapPct: 0,
        areaHa: awB?._turfAreaHa || 0,
      }
    }
    ...
    if (skanCount === 1 && awCount === 1) {
      ...
      splitFieldNames = proposalCandidates.map((c) => c.name)
      finalFieldName =
        agskanFields.find((f) => f.field_id === group.skanIds[0])?.name || ""
  ...
  function toggleCandidateSelection(id: string) {
    ...
    // dominantFieldIds.
    const splitMode = proposalDominant
      ? dominantSideChoice !== proposalDominant.source
      : false
    if (splitMode) {
      if (!wasSelected) {
  ...
  function confirmProposal() {
    ...
    const selectedCands = proposalCandidates.filter((c) =>
      selectedOverlayIds.includes(c.id),
    )
    ...
    if (proposalType === "match") {
      ...
      const boundary =
        boundaryChoice === "agskan" ? skanField?.boundary : awB?.boundary
      const area =
        boundaryChoice === "agskan"
          ? skanField?._turfAreaHa || 0
          : awB?._turfAreaHa || 0
      const name =
        finalFieldName ||
        (nameChoice === "agskan"
      ...
      consolidatedFields = [
        ...consolidatedFields,
        {
          ...
          agskan: {
            keep: boundaryChoice === "agskan" ? [skanId] : [],
            create: boundaryChoice === "agworld",
            delete: [],
          },
          agworld: {
            keep: boundaryChoice === "agworld" ? [awId] : [],
            create: boundaryChoice === "agskan",
            delete: [],
          },
        },
      ]
      ...
      const domArea =
        dom.source === "agskan"
          ...
          : agworldBoundaries.find((b) => b.field_id === dom.id)?._turfAreaHa ||
            0

      const absorbedSkanIds = selectedCands
        .filter((c) => c.source === "agskan")
        .map((c) => c.id)
      const absorbedAwIds = selectedCands
        .filter((c) => c.source === "agworld")
        .map((c) => c.id)
      ...
      consolidatedFields = [
        ...consolidatedFields,
        {
          ...
          category: "merged" as const,
          agskan: {
            keep: dom.source === "agskan" ? [dom.id] : [],
            create: dom.source === "agworld" && absorbedSkanIds.length > 0,
            delete: dom.source === "agskan" ? [] : absorbedSkanIds,
          },
          agworld: {
            keep: dom.source === "agworld" ? [dom.id] : [],
            create: dom.source === "agskan" && absorbedAwIds.length > 0,
            delete: dom.source === "agworld" ? [] : absorbedAwIds,
          },
        },
      ]
      ...
      for (let i = 0; i < selectedCands.length; i++) {
        ...
        const candName =
          splitFieldNames[
            proposalCandidates.findIndex((c) => c.id === cand.id)
          ] || cand.name
        ...
        consolidatedFields = [
          ...consolidatedFields,
          {
            ...
            agskan: {
              ...
              delete:
                domSource === "agskan" && shouldDeleteDom ? [domDeleteId] : [],
            },
            agworld: {
              keep: cand.source === "agworld" ? [cand.id] : [],
              create: domSource === "agworld" && shouldDeleteDom,
              delete:
                domSource === "agworld" && shouldDeleteDom ? [domDeleteId] : [],
            },
          },
        ]
...
<div class="agworld-container agw-page">
  ...
  <div class="page-header">
    <div class="header-left">
      ...
      <div>
        ...
        <p class="header-subtitle">
          Manage Agworld API accounts, import field boundaries, and match spray
          records
        </p>
      </div>
    </div>
    {#if accounts.length > 0}
      <div class="header-right">
        <span class="agw-badge agw-badge-info"
    ...
    {#if accounts.length === 0}
      <div class="agw-empty bg-base-200">
        <Icon
          ...
          width="48"
          height="48"
          class="agw-empty-icon"
        />
        <p class="agw-empty-title">No Agworld accounts configured</p>
        <p class="agw-empty-subtitle">
          Add an account using an API key to get started.
        </p>
    ...
    {:else}
      ...
      <div class="acct-grid">
        {#each accounts as account (account.id)}
          <div
            class="acct-card agw-card bg-base-200"
              ...
              <div class="flex items-start justify-between">
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="agw-badge agw-badge-sm agw-badge-outline"
                      >{AGWORLD_INSTANCES[account.instance]?.label ??
                        account.instance}</span
                    >
                  </div>
                  <p class="acct-card-key" title={account.apiKey}>
                    {account.apiKey.substring(0, 24)}...
                  </p>
                  <p class="acct-card-meta">
                    Added {new Date(account.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <button
                  class="agw-btn agw-btn-danger agw-btn-xs ml-2 shrink-0"
  ...
  {:else if view === "add"}
    <div style="max-width:500px">
      <button
        class="agw-btn agw-btn-ghost agw-btn-sm mb-4"
        on:click={backToList}
      >
      ...
      <div class="agw-card bg-base-200">
        <div class="agw-card-body">
          ...
          <p class="mb-4 text-xs opacity-50">
            Enter a name, API key, and select the Agworld instance. The API key
            must use the <code class="agw-badge agw-badge-sm">v1_...</code> format.
          </p>

          <div class="mb-3">
            <label
              class="mb-1 block text-xs font-medium opacity-60"
              for="account-name">Account Name</label
            >
            <input
              id="account-name"
              type="text"
          ...
          <div class="mb-3">
            <label
              class="mb-1 block text-xs font-medium opacity-60"
              for="agworld-instance">Instance</label
            >
            <select
              id="agworld-instance"
              class="agw-select"
          ...
          <div class="mb-3">
            <label
              class="mb-1 block text-xs font-medium opacity-60"
              for="grower-id-field">Grower ID (optional)</label
            >
            <input
              id="grower-id-field"
              type="text"
          ...
          <div class="mb-3">
            <label
              class="mb-1 block text-xs font-medium opacity-60"
              for="api-key"
              >API Key <span class="font-normal opacity-40"
                >v1_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</span
              ></label
            >
            <input
              id="api-key"
          ...
          <div class="mt-4 flex justify-end gap-2">
            <button class="agw-btn agw-btn-ghost" on:click={backToList}
              >Cancel</button
            >
            <button
              class="agw-btn agw-btn-primary"
  ...
  {:else if view === "account" && selectedAccount}
    <div>
      ...
      <!-- Account info bar -->
      <div class="agw-card mb-5 bg-base-200">
        <div class="agw-card-body">
          <div class="flex flex-wrap items-center gap-4">
            <div>
              ...
              <p
                class="mt-1 font-mono text-xs opacity-50"
                title={selectedAccount.apiKey}
              >
                Key: {selectedAccount.apiKey.substring(0, 24)}...
              </p>
              {#if selectedAccount.growerId}
                <p class="mt-1 font-mono text-xs opacity-50">
            ...
            <div class="ml-auto flex gap-2">
              <button
                class="agw-btn agw-btn-primary agw-btn-sm"
              ...
              {#if hasSavedConsolidation()}
                <button
                  class="agw-btn agw-btn-ghost agw-btn-sm"
                  on:click={() => {
                    if (loadConsolidationMetadata()) {
                      view = "compare"
                      toast.success(
                        `Loaded ${consolidatedFields.length} consolidated fields`,
                      )
                    } else {
                      toast.error("Failed to load saved consolidation")
                    }
                  }}
                >
                  <Icon
              ...
              {/if}
              <button
                class="agw-btn agw-btn-danger agw-btn-sm"
                on:click={() => {
      ...
      <div class="agw-tabs">
        <button
          class="agw-tab"
          class:active={accountTab === "explorer"}
          on:click={() => (accountTab = "explorer")}
        >
          <Icon icon="solar:code-bold" width="14" height="14" />API Explorer
        </button>
        <button
          class="agw-tab"
          class:active={accountTab === "records"}
          on:click={() => {
            accountTab = "records"
            loadAgworldFieldList()
            loadAgworldRecords()
      ...
      {#if accountTab === "explorer"}
        ...
        <div class="agw-card mb-5 bg-base-200">
          <div class="agw-card-body">
            ...
            <p class="mb-3 text-xs opacity-50">
              Compare data always uses this specific customer's map.
            </p>

            {#if selectedAccount.linkedMapId}
              <div
                class="flex items-center gap-2"
                style="background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.15);border-radius:10px;padding:12px 14px"
              >
                <Icon
                ...
                <div class="min-w-0 flex-1">
                  <div class="truncate text-sm font-semibold">
                    {selectedAccount.linkedProfileLabel || "Linked"}
                  </div>
                ...
                <button
                  class="agw-btn agw-btn-ghost agw-btn-xs"
                  on:click={unlinkAccount}>Unlink</button
                >
              </div>
            {:else}
              <div class="agw-alert agw-alert-error mb-2">
                ...
                />
                <span class="text-xs"
                  >Not linked — compare will fall back to your own logged-in
                  map.</span
                >
              </div>
              <div class="flex gap-2">
                <input
                  ...
                  on:keydown={(e) => {
                    if (e.key === "Enter") searchAgskanProfiles()
                  }}
                />
                <button
                  class="agw-btn agw-btn-primary agw-btn-sm"
                  on:click={searchAgskanProfiles}
                  disabled={linkSearching}
                >
                  {#if linkSearching}<span class="agw-spinner"
                    ></span>{:else}<Icon
              ...
              {#if linkSearchResults.length > 0}
                ...
                >
                  {#each linkSearchResults as p (p.id)}
                    <button
                      class="flex w-full items-center justify-between gap-2 rounded-lg border border-base-300 p-2 text-left hover:bg-base-300/50"
                    ...
                    >
                      <div class="min-w-0">
                        ...
                        <div class="truncate text-xs opacity-50">{p.email}</div>
                      </div>
                      {#if p.master_map_id}
                        <span
                          class="agw-badge agw-badge-sm agw-badge-success shrink-0"
                          >has map</span
                        >
                      {:else}
                        <span
                          class="agw-badge agw-badge-sm agw-badge-outline shrink-0"
                          >no map</span
                        >
                      {/if}
                    </button>
        ...
        <div class="grid gap-5 lg:grid-cols-3">
          ...
          <div class="space-y-4 lg:col-span-1">
            <div class="agw-card bg-base-200">
              <div class="agw-card-body">
                ...
                <input
                  type="text"
                  class="agw-input"
                  style="font-family:monospace;font-size:14px;padding:8px 12px"
                  placeholder="CPY-AU-..."
                ...
                <div class="query-grid">
                  {#each QUICK_QUERIES as q}
                    <button
                      class="query-btn"
                      class:active={queryEndpoint === q.endpoint}
                      style={queryEndpoint === q.endpoint
                        ? "border-color:rgba(59,130,246,0.5);color:#60a5fa;font-weight:600"
                        : ""}
                      on:click={() => {
                        queryEndpoint = q.endpoint
                        runQuery(q.endpoint)
                      }}
                      disabled={apiLoading}
                    >
                      {#if apiLoading && queryEndpoint === q.endpoint}
                        <span
                          class="agw-spinner"
                          style="width:12px;height:12px;border-width:2px;display:inline-block;margin-right:4px"
                        ></span>
                      {/if}
                      {q.label}
                      {#if q.needsGrower}<span
                          class="ml-1 text-[10px] opacity-50">grower</span
                        >{/if}
                    </button>
                  {/each}
                </div>
            ...
            <div class="agw-card mt-3 bg-base-200">
              <div class="agw-card-body">
                <h3 class="agw-card-title mb-2">
                  <Icon
                ...
                <div class="space-y-2 text-xs opacity-50">
                  <p>
                    <strong>Base:</strong>
                    <span
                      class="agw-badge agw-badge-sm"
                      style="font-family:monospace"
                      >{AGWORLD_INSTANCES[selectedAccount.instance]
                        ?.base}/api/v3</span
                    >
                  </p>
                  <p>
                    ...
                    <span
                      class="agw-badge agw-badge-sm"
                      style="font-family:monospace">API-Key v1_...</span
                    >
                  </p>
            ...
            <div class="agw-card bg-base-200">
              <div class="agw-card-body">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    {#if apiStatus !== null}
                      <span
                        class="agw-badge agw-badge-sm"
                        class:agw-badge-success={apiStatus >= 200 &&
                          apiStatus < 300}
                        class:agw-badge-error={apiStatus >= 400}
                      >
                        HTTP {apiStatus}
                      </span>
                    {/if}
                ...
                <div class="mt-3">
                  ...
                  {:else if apiResult}
                    <pre
                      class="overflow-auto rounded-lg bg-base-300 p-4 text-xs"
                      style="max-height:500px"><code
                        >{formatJson(apiResult)}</code
                      ></pre>
                  {:else}
                    <div
                      class="agw-empty"
                      style="background:transparent;padding:32px 0"
                    >
                      <Icon
                        icon="solar:cloud-arrow-up-bold-duotone"
                        width="40"
                        height="40"
                        class="agw-empty-icon"
                      />
                      <p class="agw-empty-subtitle">
                        Run a query to see the API response
                      </p>
                    </div>
                  {/if}
                </div>
              ...
              <div class="agw-card bg-base-200">
                <div class="agw-card-body">
                  <div class="mb-3 flex items-center justify-between">
                    ...
                    <button
                      class="agw-btn agw-btn-danger agw-btn-xs"
                      on:click={clearLogs}
                    >
                      <Icon
                    ...
                    {#each requestLogs as log (log.id)}
                      <div class="log-entry">
                        ...
                        >
                          <span
                            class="agw-badge agw-badge-sm"
                            class:agw-badge-success={log.status &&
                              log.status < 300}
                            class:agw-badge-error={log.status &&
                              log.status >= 400}
                          >
                            {log.status ?? "ERR"}
                          </span>
                          ...
                          <code class="flex-1 truncate text-xs"
                            >{log.method} {log.url}</code
                          >
                          <span class="text-xs opacity-50"
                            >{log.durationMs}ms</span
                          >
                          <Icon
                            icon={expandedLogId === log.id
                        ...
                        {#if expandedLogId === log.id}
                          <div class="log-detail space-y-3 text-xs">
                            <div>
                              <div
                              ...
                              <div class="space-y-1 rounded bg-base-100 p-2">
                                <div>
                                  <span class="opacity-50">Time:</span>
                                  {log.timestamp}
                                </div>
                                <div>
                                  <span class="opacity-50">API:</span>
                                  {log.apiVersion} · {log.instance}
                                </div>
                                <div>
                                  <span class="opacity-50">URL:</span>
                                  <code class="break-all">{log.url}</code>
                                </div>
                                <div class="mt-1">
                                  <span class="opacity-50">Headers:</span>
                                </div>
                            ...
                            <div>
                              <div
                                class="mb-1 font-semibold"
                                class:text-success={log.status &&
                                  log.status < 300}
                                class:text-error={log.status &&
                                  log.status >= 400}
                              >
                                ◀ RESPONSE ({log.status}
                                {log.statusText ?? ""}) — {log.durationMs}ms
                              </div>
                              <div class="space-y-1 rounded bg-base-100 p-2">
                                {#if log.error}
                                  <div class="mb-1 text-error">
                                    Error: {log.error}
                                  </div>
                                {/if}
                                ...
                                {#if log.responseBody}
                                  <details class="mt-2">
                                    <summary
                                      class="cursor-pointer opacity-50 hover:opacity-100"
                                      >Response Body ({log.responseBody.length} chars)</summary
                                    >
                                    <pre
                                      class="mt-2 max-h-[300px] overflow-auto rounded bg-base-300 p-2"><code
                                        >{log.responseBody.length > 2000
                                          ? log.responseBody.substring(
                                              0,
                                              2000,
                                            ) + "\n... (truncated, see copy)"
                                          : log.responseBody}</code
                                      ></pre>
                                  </details>
                                {/if}
                              </div>
      ...
      {:else}
        ...
        <div class="agworld-records">
          ...
          <div class="page-header">
            <div class="header-left">
              ...
              <div>
                ...
                <p class="header-subtitle">
                  {agworldRecords.length} record{agworldRecords.length !== 1
                    ? "s"
                    : ""}
                  · {agworldRecords
                    .reduce((s, r) => s + (r.area_hectares || 0), 0)
                    .toFixed(1)} ha total · {agworldRecords
                    .reduce((s, r) => s + (r.distance_km || 0), 0)
                    .toFixed(1)} km traveled
                </p>
              </div>
            </div>
            <div class="header-right">
              <button
                ...
                on:click={importAllAgworldFields}
              ...
              <button
                class="action-btn action-btn-accent"
                on:click={generateAgworldRecords}
                disabled={recordsLoading}
                title="Generate records from the next batch of unprocessed trails"
              >
                {#if recordsLoading}
                  <span class="loading loading-spinner loading-xs"></span>
                {:else}
              ...
              {#if trailProgress.total > 0}
                <span
                  class="count-badge"
                  title="Backfilled back to {trailProgress.oldest_processed
                    ? new Date(
                        trailProgress.oldest_processed,
                ...
                >
                  ...
                  />
                  <span
                    >{trailProgress.processed}/{trailProgress.total} trails</span
                  >
              ...
              <button
                ...
                on:click={clearAgworldData}
                disabled={recordsLoading || savingConsolidated}
                title="Delete all agworld fields & records"
              >
              ...
              <div class="count-badge">
                ...
                />
                <span
                  >{agworldFieldCount} field{agworldFieldCount !== 1
                    ? "s"
                    : ""}</span
                >
              </div>
          ...
          <div class="filters-bar">
            <div class="search-box border border-base-300 bg-base-100">
              <Icon
            ...
            {#if true}
              {@const years = [
                ...new Set(agworldFieldList.map((f) => f.production_year)),
              ]
                .filter(Boolean)
                .sort((a, b) => b - a)}
              {#if years.length > 0}
                <div class="year-chips">
                  {#each years as y}
                    <button
                      class="year-chip border border-base-300 bg-base-100"
                  ...
                  {#if selectedYear}
                    <button
                      class="year-chip year-chip-clear"
                      on:click={() => (selectedYear = null)}>✕ Clear</button
                    >
                  {/if}
                </div>
              {/if}
          ...
          {#if showMap && overlayField}
            <FieldTrailOverlay
              ...
              inputProducts={overlayInputProducts}
              fieldCrop={overlayCrop}
              agworldActivities={fieldActivityData.get(overlayField.id) || null}
              on:close={() => {
                showMap = false
                overlayField = null
              }}
          ...
          {#if farmGroups.length > 0}
            {@const syncableFields = agworldFieldList.filter(
              (f) => f.properties?.agworld_boundary_id || f.agworld_field_ref,
            ).length}
            {@const syncedFields = agworldFieldList.filter(
              (f) => f.last_activity_sync_at,
            ).length}
              ...
              <div class="activity-banner-info">
                ...
                {:else if syncedFields > 0}
                  <span>
                    ...
                    <span class="opacity-60">
                      — {syncedFields} field{syncedFields !== 1 ? "s" : ""} synced
                      · last: {new Date(
                        Math.max(
                          ...agworldFieldList
                            .filter((f) => f.last_activity_sync_at)
                            .map((f) =>
                              new Date(f.last_activity_sync_at).getTime(),
                            ),
                        ),
                      ).toLocaleDateString("en-AU", {
                        day: "numeric",
                ...
                {:else}
                  <span>
                    <strong>Agworld Activity Data</strong>
                    <span class="opacity-60">
                      — Click <span class="activity-btn-inline">Activity</span>
                      on a field or
                      <span class="activity-btn-inline">Sync All</span> to pull actuals,
                      work orders &amp; plans</span
                    >
                  </span>
                {/if}
              </div>
              ...
              <div class="fields-grouped">
                {#if agworldRecords.length === 0}
                  ...
                  >
                    ...
                    />
                    <p class="agw-empty-subtitle">
                      No spray records matched yet. Click "Match Spray Records"
                      to find which spray jobs overlap these fields.
                    </p>
                  </div>
                {/if}
                {#each farmGroups as farmGroup (farmGroup.name)}
                  <div class="field-group border border-base-300 bg-base-200">
                    <div
                      class="field-group-header"
                      on:click={() => toggleFarmExpand(farmGroup.name)}
                    >
                      <Icon
                      ...
                      <div class="field-group-info">
                        ...
                        <span class="field-group-count"
                          >{farmGroup.fields.length} field{farmGroup.fields
                            .length !== 1
                            ? "s"
                            : ""} · {farmGroup.totalHa.toFixed(1)} ha</span
                        >
                      </div>
                    ...
                    {#if expandedFarms.has(farmGroup.name)}
                      <div class="field-group-fields">
                        {#each farmGroup.fields as field (field.id)}
                          <div
                            class="field-card border border-base-300 bg-base-100"
                            class:selected={selectedFieldId === field.id}
                          >
                            <div
                              class="field-card-header"
                              on:click={() => toggleFieldExpand(field.id)}
                            >
                              <div
                                class="field-thumb-wrap"
                                on:click|stopPropagation={() =>
                                  openFieldOverlay(field)}
                                role="button"
                                tabindex="0"
                                title="View on map"
                              >
                                <FieldOutlineThumbnail
                              ...
                              <div class="field-card-info">
                                ...
                                <div class="field-card-meta">
                                  {field.area
                                    ...
                                    : "-"}
                                  {#if field.crop_type}<span class="crop-badge"
                                      >{field.crop_type}</span
                                    >{/if}
                                  {#if field.production_year}<span
                                      class="year-tag"
                                      >{field.production_year}</span
                                    >{/if}
                                </div>
                              </div>
                              <div class="field-card-actions">
                                <span class="record-count-badge"
                                  >{agworldRecords.filter(
                                    (r) => r.agworld_field_id === field.id,
                                  ).length} record{agworldRecords.filter(
                                    (r) => r.agworld_field_id === field.id,
                                  ).length !== 1
                                    ? "s"
                                    : ""}</span
                              ...
                              <div class="field-records">
                                {#each agworldRecords.filter((r) => r.agworld_field_id === field.id) as rec (rec.id)}
                                  <div class="record-row">
                                    ...
                                    <span class="record-meta">
                                      <Icon
                                        icon="solar:tractor-bold"
                                        width="11"
                                        height="11"
                                        class="opacity-30"
                                      />
                                      {rec.vehicle_type ||
                                        rec.vehicle_marker?.type ||
                                        "-"}
                                    </span>
                                    <span class="record-stat"
                                      >{rec.area_hectares
                                        ? rec.area_hectares.toFixed(1) + " ha"
                                        : "-"}</span
                                    >
                                    <span class="record-stat"
                                      >{rec.distance_km
                            ...
                            {#if expandedActivityFields.has(field.id)}
                              {@const activity = fieldActivityData.get(
                                field.id,
                              )}
                              <div class="field-activity">
                                ...
                                {:else if activity}
                                  ...
                                  {@const totalItems =
                                    tl.convertedPairs.length +
                                    tl.unmatchedActuals.length +
                                    tl.pendingPlans.length +
                                    tl.workOrders.length}
                                  {#if totalItems === 0}
                                    <div class="activity-empty">
                                      No actuals, work orders, or plans found
                                      for this field.
                                    </div>
                                  {:else}
                                      ...
                                      <div class="activity-group">
                                        <div class="activity-group-header">
                                          ...
                                          <span class="activity-count"
                                            >{tl.convertedPairs.length}</span
                                          >
                                        </div>
                                        {#each tl.convertedPairs as pair (pair.actual.id)}
                                          {@const aAttrs =
                                            pair.actual.attributes || {}}
                                          <div
                                            class="activity-row converted"
                                            on:click|stopPropagation={() =>
                                              openActivityModal(
                                                pair.actual.id,
                                                pair.actual,
                                          ...
                                          >
                                            ...
                                            <span class="activity-lifecycle">
                                              <span class="lifecycle-badge plan"
                                                >Plan</span
                                              >
                                              <Icon
                                      ...
                                      <div class="activity-group">
                                        ...
                                        {#each tl.discardedConvertedPairs as pair (pair.actual.id)}
                                          {@const aAttrs =
                                            pair.actual.attributes || {}}
                                          <div
                                            class="activity-row discarded-converted"
                                            on:click|stopPropagation={() =>
                                              openActivityModal(
                                                pair.actual.id,
                                                pair.actual,
                                          ...
                                          >
                                            ...
                                            <span class="activity-lifecycle">
                                              <span
                                                class="lifecycle-badge discarded-plan"
                                                >Discarded</span
                                              >
                                              <Icon
                                                icon="solar:arrow-right-bold"
                                    ...
                                    {#if tl.unmatchedActuals.length > 0}
                                      <div class="activity-group">
                                        <div class="activity-group-header">
                                          ...
                                          <span>Actuals</span>
                                          <span class="activity-count"
                                            >{tl.unmatchedActuals.length}</span
                                          >
                                        </div>
                                        {#each tl.unmatchedActuals as a (a.id)}
                                          ...
                                          >
                                            ...
                                            >
                                            <span
                                              class="activity-type-badge actual"
                                              >Actual</span
                                            >
                                            <span
                                              class="activity-status"
                                              class:completed={attrs.status ===
                                                "completed"}
                                              >{attrs.status || "-"}</span
                                            >
                                            <span class="activity-name"
                                              >{attrs.name ||
                                                attrs.operation_type_name ||
                                                "-"}</span
                                            >
                                          </div>
                                        {/each}
                                      ...
                                      <div class="activity-group">
                                        ...
                                        {#each tl.workOrders as wo (wo.id)}
                                          ...
                                          >
                                            ...
                                            >
                                            <span
                                              class="activity-type-badge work-order"
                                              >W/O</span
                                            >
                                            <span
                                              class="activity-status"
                                      ...
                                      <div class="activity-group">
                                        ...
                                        {#each tl.pendingPlans as p (p.id)}
                                          ...
                                          >
                                            ...
                                            >
                                            <span
                                              class="activity-type-badge plan"
                                              >Plan</span
                                            >
                                            {#if isOverdue}
                                      ...
                                      <div class="activity-group">
                                        ...
                                        {#each tl.discardedPlans as p (p.id)}
                                          ...
                                          <div class="activity-row discarded">
                                            ...
                                            >
                                            <span
                                              class="activity-type-badge discarded-plan"
                                              >Discarded</span
                                            >
                                            <span
                                              class="activity-status discarded-status"
                                              >{attrs.status || "-"}</span
                                            >
                                            <span
                                              class="activity-name discarded-name"
                                              >{attrs.name ||
                                                attrs.operation_type_name ||
                                                "-"}</span
                                            >
            ...
            {:else}
              ...
              <div class="empty-state">
                ...
                <p class="empty-subtitle">
                  Click "Import Agworld Fields" to pull field boundaries from
                  Agworld, then "Match Spray Records" to find which AgSKAN spray
                  jobs fall within each Agworld field.
                </p>
              </div>
            {/if}
          {/if}
    ...
    {#if activityModal && activityModalData}
      ...
      <dialog
        bind:this={activityDialogEl}
        class="modal modal-bottom sm:modal-middle"
        on:close={closeActivityModal}
      >
        <div
          class="modal-box flex max-h-[90vh] max-w-5xl flex-col overflow-y-auto p-0"
        >
          <div
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="flex h-9 w-9 items-center justify-center rounded-full bg-blue-600/20"
              >
                <Icon
              ...
              <div>
                <h3 class="text-base font-semibold text-contrast-content">
                  {attrs.name || attrs.operation_type_name || m.type}
                </h3>
                <p
                  class="flex items-center gap-2 text-xs text-contrast-content/60"
                >
                  {m.type}
                  {#if attrs.status}<span
                      class="badge badge-sm text-xs font-semibold capitalize"
                      class:badge-success={attrs.status === "completed" ||
                        attrs.status === "closed"}
                      class:badge-warning={attrs.status === "todo" ||
                        attrs.status === "in_progress"}
                      >{attrs.status.replace(/_/g, " ")}</span
                    >{/if}
                </p>
              </div>
            </div>
          ...
          <!-- Layout Toggle -->
          <div class="flex items-center justify-center gap-1 px-6 pb-2">
            {#each ["A", "B", "C"] as opt}
              <button
                class="btn btn-xs h-7 min-h-0 w-9 font-mono text-xs font-semibold {activityLayout ===
                opt
                  ? 'btn-primary'
                  : 'btn-ghost'}"
                on:click={() => setActivityLayout(opt)}>{opt}</button
              >
            {/each}
          </div>
          ...
          <div class="flex-1 space-y-4 overflow-y-auto px-6 py-4">
            ...
            {:else if activityLayout === "A"}
              {#if d.activityAttrs?.completed_at}
                <span
                  class="badge badge-ghost badge-sm text-[11px] font-normal text-contrast-content/60"
                >
                  <Icon
                    ...
                    class="mr-1"
                  />
                  {new Date(d.activityAttrs.completed_at).toLocaleDateString(
                    "en-AU",
              ...
              {#if d.farmName || d.fieldNames?.length}
                <div
                  class="rounded-xl border border-success/15 bg-success/[0.03] p-4"
                >
                  <table class="w-full text-xs text-contrast-content">
                    <thead
                      ><tr
                        class="text-[10px] font-semibold uppercase text-contrast-content/35"
                        ><th class="pb-1 pr-2 text-left">Farm</th><th
                          class="pb-1 pr-2 text-left">Field</th
                        >{#if d.fieldCropName}<th class="pb-1 pr-2 text-left"
                            >Crop &amp; Variety</th
                          >{/if}<th class="pb-1 pr-2 text-left">Field ha</th><th
                          class="pb-1 text-left">Activity ha</th
                        ></tr
                      ></thead
                    >
                    <tbody
                      ><tr>
                        <td class="pr-2 font-semibold">{d.farmName || "-"}</td>
                        <td class="pr-2 font-semibold"
                          >{d.fieldNames?.join(", ") || "-"}</td
                        >
                        {#if d.fieldCropName}<td class="pr-2 font-semibold"
                            >{d.fieldCropName}</td
                          >{/if}
                        <td class="pr-2">{d.areaHa?.toFixed(2) || "-"} ha</td>
                        <td>{d.activityAttrs?.area?.scalar || "-"} ha</td>
                      </tr></tbody
                    >
                  </table>
                </div>
              ...
              {#if d.seedings.length > 0 || d.products.length > 0}
                <div
                  class="overflow-hidden rounded-xl border border-base-300 bg-base-200/30"
                >
                  <div
                    class="border-b border-base-300 bg-base-200/50 px-4 py-2.5"
                  >
                    <span
                      class="text-xs font-bold uppercase text-contrast-content/50"
                      >Inputs</span
                    >
                  </div>
                  <table class="w-full text-xs text-contrast-content">
                    <thead
                      ><tr
                        class="text-[10px] font-semibold uppercase text-contrast-content/35"
                        ><th class="py-1.5 pl-4 text-left">Input</th><th
                          class="py-1.5 text-left">Rate</th
                        ><th class="py-1.5 text-left">Total Qty</th></tr
                      ></thead
                    >
                    <tbody>
                      {#each [...d.seedings, ...d.products] as item, idx}
                        {#if idx < d.seedings.length}
                          {@const s = item.attributes || {}}
                          {@const variety = d.seedingVarieties.find(
                            (v) =>
                              v.id ===
                              item.relationships?.crop_variety?.data?.id,
                          )}
                          <tr class="border-t border-base-300/20">
                            <td class="py-2 pl-4"
                              ...
                              {#if variety?.attributes?.crop_type?.name}<div
                                  class="text-[10px] uppercase text-contrast-content/40"
                                >
                                  {variety.attributes.crop_type.name}
                                </div>{/if}</td
                            >
                            <td class="py-2"
                              >{#if s.application_rate?.scalar}{s
                                  .application_rate.scalar}
                                {s.application_rate.units || ""}{/if}</td
                            >
                            <td class="py-2 font-semibold"
                              >{#if s.total_weight?.scalar}{s.total_weight
                                  .scalar}
                                {s.total_weight.units || ""}{/if}</td
                            >
                          </tr>
                        {:else}
                          {@const p = item.attributes || {}}
                          {@const r = p.application_rate}
                          {@const v = p.total_volume}
                          {@const w = p.total_weight}
                          {@const amt =
                            v?.scalar > 0
                              ? v.scalar
                              : w?.scalar > 0
                          ...
                          {@const un =
                            v?.scalar > 0
                              ...
                              : w?.scalar > 0
                                ? w.units
                                : ""}
                          {@const pd = d.productDetails.find(
                            (x) =>
                              x.id === item.relationships?.product?.data?.id,
                          )}
                          {@const ings =
                            pd?.attributes?.active_ingredients || []}
                          <tr class="border-t border-base-300/20">
                            <td class="py-2 pl-4">
                              <div class="font-semibold">
                                {p.name || "Unknown"}
                              </div>
                              {#if pd?.attributes?.category}<div
                                  class="text-[10px] text-contrast-content/40"
                                >
                                  {pd.attributes
                                    .category}{#if pd.attributes.formulation}
                                    - {pd.attributes.formulation}{/if}
                                </div>{/if}
                              {#if ings.length > 0}<div
                                  class="text-[10px] text-contrast-content/50"
                                >
                                  {#each ings as ing, j}{@const isc =
                                      ing.content?.scalar || 0}{@const sub =
                                      pd?.attributes?.substance}{@const den =
                                      pd?.attributes?.density?.scalar ||
                                      0}{@const dv =
                                      sub === "solid"
                                        ? (isc * 10).toFixed(0) + " g/kg"
                                        : sub === "liquid" && den > 0
                                          ? (isc * den * 10).toFixed(0) + " g/L"
                                          : isc +
                                            (ing.content?.units || "%")}<span
                                      >{ing.name}: {dv}{j < ings.length - 1
                                        ? ", "
                                        : ""}</span
                                    >{/each}
                                </div>{/if}
                            ...
                            <td class="py-2"
                              >{#if r?.scalar}{r.scalar}
                                {r.units || ""}{/if}</td
                            >
                            <td class="py-2 font-semibold"
                              >{#if amt > 0}{amt} {un}{/if}</td
                            >
                          </tr>
                        {/if}
                      {/each}
                ...
                >
                  ...
                  <table class="w-full text-xs text-contrast-content">
                    <thead
                      ><tr
                        class="text-[10px] font-semibold uppercase text-contrast-content/35"
                        ><th class="py-1.5 pl-4 text-left">Operation</th><th
                          class="py-1.5 text-left">Rate</th
                        ><th class="py-1.5 text-left">Total Qty</th></tr
                      ></thead
                    >
                    <tbody>
                      {#each d.operations as op}
                        {@const oa = op.attributes || {}}
                        <tr class="border-t border-base-300/20">
                          <td class="py-2 pl-4 font-semibold"
                            >{oa.name ||
                              oa.operation_type_name ||
                              "Unknown"}</td
                          >
                          <td class="py-2"
                            >{#if oa.application_rate?.scalar}{oa
                                .application_rate.scalar}
                              {oa.application_rate.units || ""}{/if}</td
                          >
                          <td class="py-2 font-semibold"
                            >{#if oa.total_area?.scalar}{oa.total_area.scalar}
                              {oa.total_area.units ||
                                ""}{:else if oa.area?.scalar}{oa.area.scalar}
                              {oa.area.units || ""}{/if}</td
                          >
                        </tr>
                      {/each}
              ...
              {#if d.nutritionTotal?.length || d.activityAttrs}
                <div
                  class="space-y-2 rounded-xl border border-base-300 bg-base-200/20 p-4 text-xs"
                >
                  {#if d.nutritionTotal?.length}
                    <div class="text-contrast-content/70">
                      <span class="font-semibold text-contrast-content"
                        >Activity Nutrition (kg/ha):</span
                      >
                      {#each d.nutritionTotal as { name, value }, i}<span
                          class="font-semibold text-contrast-content"
                          >{name}</span
                        >
                        {value.toFixed(3)}{i < d.nutritionTotal.length - 1
                          ? " | "
                          : ""}{/each}
                    </div>
                  {/if}
                  {#if d.activityAttrs}
                    {@const aa = d.activityAttrs}
                    <div class="space-y-0.5 text-contrast-content/50">
                      {#if aa.total_application_rate?.scalar}<div
                          class="flex gap-2"
                        >
                          <span class="w-40 text-contrast-content/40"
                            >Total Application Rate</span
                          ><span class="font-medium text-contrast-content"
                            >{aa.total_application_rate.scalar}
                            {aa.total_application_rate.units || ""}</span
                          >
                        </div>{/if}
                      {#if d.areaHa}<div class="flex gap-2">
                          <span class="w-40 text-contrast-content/40"
                            >Total Field Area</span
                          ><span class="font-medium text-contrast-content"
                            >{d.areaHa.toFixed(2)} ha</span
                          >
                        </div>{/if}
                      {#if aa.area?.scalar}<div class="flex gap-2">
                          <span class="w-40 text-contrast-content/40"
                            >Activity Area</span
                          ><span class="font-medium text-contrast-content"
                            >{aa.area.scalar} ha</span
                          >
                        </div>{/if}
                      {#if aa.area?.scalar && aa.total_application_rate?.scalar}<div
                          class="flex gap-2"
                        >
                          <span class="w-40 text-contrast-content/40"
                            >Total Amount</span
                          ><span class="font-medium text-contrast-content"
                            >{(
                              aa.area.scalar * aa.total_application_rate.scalar
                            ).toLocaleString()} L</span
                          >
                        </div>{/if}
                    </div>
                  {/if}
              ...
              {#if d._debug}
                <details class="text-xs">
                  <summary
                    class="cursor-pointer text-[11px] font-bold uppercase text-contrast-content/40"
                    >Debug Panel</summary
                  >
                  <div class="mt-2 font-mono text-[11px]">
                    <div class="mb-2">
                      <div class="font-semibold">API Calls:</div>
                      {#each Object.entries(d._debug.apiResults) as [name, r]}
                        <div>
                          <span
                            class={r.success ? "text-success" : "text-error"}
                            >{r.success ? "OK" : "FAIL"}</span
                          >
                          {name}: status={r.status}, count={r.count}
                        </div>
                      {/each}
                    </div>
                    <div>
                      <div class="font-semibold">Activity Attributes:</div>
                      <pre class="surf-json max-h-[100px]">{JSON.stringify(
                          d.activityAttrs,
                          null,
                          2,
                        )}</pre>
              ...
              {#if [...d.products, ...d.operations, ...d.operatorUsers, ...d.assets, ...d.weatherRecords].length > 0}
                {@const allSubs = [
                  ...d.products.map((r) => ({ label: "Product", r })),
                  ...d.operations.map((r) => ({ label: "Operation", r })),
                  ...d.operatorUsers.map((r) => ({ label: "Operator", r })),
                  ...d.assets.map((r) => ({ label: "Asset", r })),
                  ...d.weatherRecords.map((r) => ({ label: "Weather", r })),
                ]}
                <details class="text-xs">
                  <summary
                    class="cursor-pointer text-[11px] font-bold uppercase text-contrast-content/40"
                    >Raw API Data ({allSubs.length})</summary
                  >
                      ...
                      <div class="surf-card">
                        <div class="surf-header">
                          <span class="font-semibold"
                            >{sub.label}: {sub.r.attributes?.name ||
                              sub.r.id?.substring(0, 12)}</span
                          >
                        </div>
                        <details>
                          <summary
                            class="cursor-pointer text-[10px] text-contrast-content/50"
                            >JSON</summary
                          >
                          <pre class="surf-json">{JSON.stringify(
                              sub.r,
                              (k, v) => (k === "links" ? undefined : v),
                              2,
                            )}</pre>
                        </details>
                      </div>
            ...
            {:else}
              ...
              {#if activityLayout === "B"}
                <!-- Sub-layout picker -->
                <div class="mb-3 flex items-center justify-center gap-0.5">
                  {#each [1, 2, 3] as n}
                    <button
                      class="btn btn-xs h-6 min-h-0 w-7 font-mono text-[10px] font-semibold {activitySubLayout ===
                      n
                        ? 'btn-primary'
                        : 'btn-ghost'}"
                      on:click={() => setActivitySubLayout(n)}>{n}</button
                    >
                  {/each}
                </div>

                <!-- ====== Layout 1: Centered Hero ====== -->
                {#if activitySubLayout === 1}
                  <div class="px-2 py-3 text-center">
                    {#if d.fieldBoundary}
                      <div class="mb-2 flex justify-center">
                        <FieldOutlineThumbnail
                          boundary={d.fieldBoundary}
                          width={52}
                          height={40}
                          fillColor="#16a34a"
                          outlineColor="#86efac"
                    ...
                    {/if}
                    <div
                      class="text-[10px] uppercase tracking-widest text-contrast-content/40"
                    >
                      {d.farmName || "Farm"}
                    </div>
                    <h2 class="mt-0.5 text-xl font-bold text-contrast-content">
                      {d.fieldNames?.join(", ") || "Field"}
                    </h2>
                    <div
                      class="mt-1 flex items-center justify-center gap-3 text-xs text-contrast-content/50"
                    >
                      {#if d.fieldCropName}<span>{d.fieldCropName}</span>{/if}
                      {#if d.areaHa != null}<span
                          class="font-semibold text-contrast-content"
                          >{d.areaHa.toFixed(2)} ha</span
                        >{/if}
                    </div>
                  ...
                  <!-- ====== Layout 2: Split Bar ====== -->
                {:else if activitySubLayout === 2}
                  <div
                    class="overflow-hidden rounded-xl border border-success/10"
                  >
                    <div
                      class="flex items-start gap-3 bg-success/5 px-4 pb-2 pt-3"
                    >
                      {#if d.fieldBoundary}
                        <FieldOutlineThumbnail
                          boundary={d.fieldBoundary}
                      ...
                      {/if}
                      <div class="min-w-0 flex-1">
                        <div class="text-[10px] text-contrast-content/40">
                          {d.farmName || "Farm"}
                        </div>
                        <h3
                          class="text-base font-bold leading-tight text-contrast-content"
                        >
                          {d.fieldNames?.join(", ") || "Field"}
                        </h3>
                      </div>
                    ...
                    >
                      <span class="text-contrast-content/50"
                        >{d.fieldCropName || ""}</span
                      >
                      {#if d.areaHa != null}<span
                          class="font-bold text-contrast-content"
                          >{d.areaHa.toFixed(2)} ha</span
                        >{/if}
                    </div>
                  </div>

                  <!-- ====== Layout 3: Left-Aligned Hero ====== -->
                {:else}
                  <div class="flex items-start gap-3 px-2 py-3">
                    ...
                    {:else}
                      <div
                        class="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-success/10"
                      >
                        <Icon
                          icon="solar:map-point-bold"
                          w="20"
                    ...
                    <div class="min-w-0">
                      ...
                      <h2
                        class="text-lg font-bold leading-tight text-contrast-content"
                      >
                        {d.fieldNames?.join(", ") || "Field"}
                      </h2>
                      <div
                        class="mt-0.5 flex items-center gap-2 text-xs text-contrast-content/50"
                      >
                        {#if d.fieldCropName}<span>{d.fieldCropName}</span>{/if}
                        {#if d.areaHa != null}<span
                            class="font-semibold text-contrast-content"
                ...
                {#if m.type === "Plan"}
                  <!-- Just the plan itself (no actual) -->
                  {@const pAttrs = d.activityAttrs || {}}
                  {@const pDue = pAttrs.due_at || pAttrs.due_date}
                  {@const pName =
                    pAttrs.name || pAttrs.operation_type_name || "Plan"}
                  <div
                    class="bg-info/3 mt-3 overflow-hidden rounded-xl border border-info/10"
                  >
                    ...
                    <div class="px-3 pb-2 text-[10px] text-contrast-content/40">
                      Plan{#if pDue}
                        &middot; Due {new Date(pDue).toLocaleDateString(
                          "en-AU",
                          { day: "numeric", month: "short", year: "2-digit" },
                        )}{/if}
                    </div>
                  </div>
                {:else if d.parentActivity || d.operations.length > 0}
                  <div
                    class="mt-3 flex flex-col gap-0 md:flex-row md:items-stretch md:gap-0"
                  >
                    <!-- Plan card -->
                    {#if d.parentActivity}
                      {@const pAttrs = d.parentActivity.attributes || {}}
                      {@const pDue = pAttrs.due_at || pAttrs.due_date}
                      {@const pName =
                        pAttrs.name ||
                        pAttrs.operation_type_name ||
                        d.parentActivity.type}
                      <div
                        class="bg-info/3 flex-1 overflow-hidden rounded-xl border border-info/10 md:rounded-r-none"
                      >
                        <div class="flex items-center gap-2 px-3 py-2">
                          <Icon
                            icon="solar:documents-bold"
                        ...
                        <div
                          class="px-3 pb-2 text-[10px] text-contrast-content/40"
                        >
                          Plan{#if pDue}
                            &middot; Due {new Date(pDue).toLocaleDateString(
                              "en-AU",
                    ...
                    <!-- Connector: arrow (desktop) / vertical line (mobile) -->
                    <div
                      class="flex shrink-0 items-center justify-center py-1 md:flex-col md:px-1 md:py-0"
                    >
                      <Icon
                        icon="solar:arrow-right-bold"
                        w="16"
                        h="16"
                        class="hidden text-contrast-content/15 md:block"
                      />
                      <Icon
                        icon="solar:arrow-down-bold"
                        w="16"
                        h="16"
                        class="text-contrast-content/15 md:hidden"
                      />
                    </div>
                    ...
                    {#if d.operations.length > 0}
                      <div
                        class="bg-success/3 flex-1 overflow-hidden rounded-xl border border-success/10 md:rounded-l-none"
                      >
                        {#each d.operations as op, oi}
                          {@const oa = op.attributes || {}}
                          {@const done =
                            oa.status === "completed" || oa.status === "closed"}
                          {@const dt = oa.completed_at || oa.created_at}
                          <div
                            class="flex items-center gap-2 px-3 py-2 {oi > 0
                              ? 'border-t border-success/5'
                              : ''}"
                          >
                            <Icon
                              icon={done
                              ...
                              h="14"
                              class={done
                                ? "shrink-0 text-success/60"
                                : "shrink-0 text-warning/60"}
                            />
                            <span
                              class="truncate text-xs font-semibold text-contrast-content"
                              >{oa.name ||
                          ...
                          >
                            Actual{#if dt}
                              &middot; {done ? "Completed" : "Started"}
                              {new Date(dt).toLocaleDateString("en-AU", {
                                day: "numeric",
                                month: "short",
                                year: "2-digit",
                              })}{/if}{#if oa.total_area}
                              &middot; {fmtAttr(oa.total_area)}{/if}
                          </div>
                        {/each}
                      </div>
                    {:else}
                      <div
                        class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-contrast-content/10 text-[10px] text-contrast-content/25"
                      >
                        No operations
                      </div>
                    {/if}
                ...
                {#if d.products.length > 0}
                  {@const totalVol = d.products.reduce(
                    (s, p) => s + (p.attributes?.total_volume?.scalar || 0),
                    0,
                  )}
                  {@const actArea = d.activityAttrs?.area?.scalar}
                  {@const firstRate =
                    d.products[0]?.attributes?.application_rate}
                  <div class="mt-3 grid grid-cols-2 gap-2">
                    {#if firstRate}
                      <div class="rounded-lg bg-base-200/30 p-2.5 text-center">
                        <div
                          class="text-[9px] font-bold uppercase text-contrast-content/35"
                        >
                          Application Rate
                        </div>
                ...
                <!-- Inputs -->
                {#if d.seedings.length > 0 || d.products.length > 0}
                  <div class="al-card mt-3">
                    <h4
                      class="mb-2 text-[11px] font-bold uppercase text-contrast-content/40"
                    >
                      Inputs ({d.seedings.length + d.products.length})
                    </h4>
                    <div class="space-y-1.5">
                      {#each [...d.seedings, ...d.products] as item, idx}
                        {#if idx < d.seedings.length}
                          ...
                          {@const rate = s.application_rate}
                          {@const amt = s.total_weight}
                          <div
                            class="flex items-start gap-2.5 rounded-lg bg-base-200/20 p-2"
                          >
                            <div
                              class="al-input-dot mt-1.5 bg-success/60"
                            ></div>
                            <div class="min-w-0 flex-1">
                              <div
                                class="text-xs font-semibold text-contrast-content"
                              >
                                {s.name || item.type}
                              </div>
                              {#if variety?.attributes?.crop_type?.name}<div
                              ...
                              <div
                                class="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-contrast-content/50"
                              >
                                {#if rate?.scalar}<span
                                    ><span class="text-contrast-content/35"
                                      >Rate:</span
                                    >
                                    {rate.scalar}
                                    {rate.units || ""}</span
                        ...
                        {:else}
                          ...
                          <div
                            class="flex items-start gap-2.5 rounded-lg bg-base-200/20 p-2"
                          >
                            <div class="al-input-dot mt-1.5 bg-info/60"></div>
                            <div class="min-w-0 flex-1">
                              <div
                                class="text-xs font-semibold text-contrast-content"
                              >
                ...
                {#if d.nutritionTotal?.length}
                  <div class="al-card">
                    ...
                    <div class="flex flex-wrap gap-2">
                      {#each d.nutritionTotal as nut}
                        <span
                          class="rounded bg-base-200/30 px-2 py-1 text-[11px] font-semibold text-contrast-content"
                          >{nut.value.toFixed(1)} kg {nut.name}/ha</span
                        >
                      {/each}
                    </div>
                  </div>
                {/if}
              {:else}
                <!-- ====== Layout C: Split Panel ====== -->
                <div
                  class="mb-3 overflow-hidden rounded-xl border border-success/10"
                >
                  <div class="bg-success/5 px-4 pb-2 pt-3">
                    <div class="text-[10px] text-contrast-content/40">
                      {d.farmName || "Farm"}
                    </div>
                    <h3
                ...
                <div class="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    {#if d.parentActivity}
                      <div class="mb-2 flex items-center gap-1.5">
                        ...
                        />
                        <span
                          class="text-[10px] font-bold uppercase tracking-wide text-contrast-content/40"
                          >Plan</span
                        >
                      </div>
                      <p
                        class="mb-1.5 ml-0.5 truncate text-[11px] text-contrast-content/50"
                      >
                        {d.parentActivity.attributes?.name ||
                          d.parentActivity.type}
                      </p>
                      <div class="-my-1 mb-1 flex justify-center">
                        <Icon
                          icon="solar:arrow-down-bold"
                          width="10"
                          height="10"
                          class="text-contrast-content/12"
                        />
                      </div>
                    {/if}
                    ...
                    <div class="al-timeline al-timeline-sm ml-1.5">
                      <div class="al-timeline-line"></div>
                      {#each d.operations as op}
                        {@const opAttrs = op.attributes || {}}
                        {@const dt = opAttrs.completed_at || opAttrs.created_at}
                        {@const isDone =
                          opAttrs.status === "completed" ||
                          opAttrs.status === "closed"}
                        <div class="al-timeline-row">
                          <div
                            class="al-timeline-dot {isDone
                              ? 'bg-success'
                              : 'bg-warning'}"
                          ></div>
                          <div class="min-w-0 flex-1">
                            ...
                            {#if dt}<p
                              ...
                              >
                                {new Date(dt).toLocaleDateString("en-AU", {
                                  day: "numeric",
                                  month: "short",
                                })}{#if opAttrs.total_area != null}
                                  &middot; {fmtAttr(opAttrs.total_area)}{/if}
                              </p>{/if}
                          </div>
                        </div>
                      {/each}
                  ...
                  <div>
                    ...
                    <div class="space-y-1.5">
                      {#each [...d.seedings, ...d.products] as item, idx}
                        {@const isSeeding = idx < d.seedings.length}
                        {@const attrs = item.attributes || {}}
                        <div
                          class="flex items-center gap-2 rounded p-1.5 text-xs"
                        >
                          <div
                            class="al-input-dot {isSeeding
                              ? 'bg-success/70'
                              : 'bg-info/70'}"
                          ></div>
                          <div class="min-w-0">
                            <span class="font-medium text-contrast-content"
                              >{attrs.name || item.type}</span
                            >
                            {#if attrs.rate != null}<span
                                class="ml-1 text-contrast-content/40"
                                >{attrs.rate}{attrs.rate_unit || ""}</span
                              >{/if}
                          </div>
                        </div>
                      {/each}
                    ...
                    {#if d.nutritionTotal?.length}
                      <h4
                        class="mb-1.5 mt-3 text-[11px] font-bold uppercase text-contrast-content/40"
                      >
                        Nutrition
                      </h4>
                      <div class="flex flex-wrap gap-1.5">
                        {#each d.nutritionTotal as nut}
                          <span
                            class="rounded bg-base-200/30 px-2 py-0.5 text-[10px] font-semibold text-contrast-content"
                            >{nut.value.toFixed(1)} kg {nut.name}/ha</span
                          >
                        {/each}
                      </div>
              ...
              {#if d._debug}
                <details class="text-xs">
                  ...
                  >
                  <pre
                    class="mt-1 overflow-x-auto text-[10px] text-contrast-content/50">{JSON.stringify(
                      d._debug,
                      null,
                      2,
                    )}</pre>
    ...
    <div>
      ...
      <div class="mb-4">
        ...
        <p class="mt-1 text-sm opacity-50">
          {selectedAccount?.name} — Grower: {selectedAccount?.growerId ||
            "not set"}
        </p>
      </div>
      ...
      {:else}
        ...
        <div class="compare-stats">
          <div class="stat-card agw-card bg-base-200">
            <div class="stat-card-value">{agskanFields.length}</div>
            <div class="stat-card-label">AgSKAN</div>
          </div>
          <div class="stat-card agw-card bg-base-200">
            <div class="stat-card-value">{agworldFields.length}</div>
            <div class="stat-card-label">Agworld</div>
          </div>
          <div
            class="stat-card bg-base-200"
            style="background:rgba(34,197,94,0.1)"
          >
            <div class="stat-card-value" style="color:#4ade80">
              {identicalPairs.length + basicallyIdenticalPairs.length}
            </div>
          ...
          >
            <div class="stat-card-value" style="color:#60a5fa">
              {autoImportedSkan.length + autoImportedAw.length}
            </div>
            <div class="stat-card-label">Imported</div>
          </div>
          <div
            class="stat-card bg-base-200"
            style="background:rgba(245,158,11,0.1)"
          >
            <div class="stat-card-value" style="color:#fbbf24">
              {totalReview}
            </div>
          ...
          >
            ...
            <div class="stat-card-label">Consolidated</div>
          </div>
        </div>
        ...
        {#if compareLogs.length > 0}
          <details class="mb-3">
            <summary class="cursor-pointer text-xs opacity-60 hover:opacity-80">
              {compareLogs.length} API calls · {agworldBoundaries.filter(
                (b) => b.boundary,
              ).length} boundaries loaded
            </summary>
          </details>
        {/if}
        ...
        {@const resolvedCombined = [
          ...identicalPairs.map((p) => ({
            key: "id-" + p.skan.field_id,
            name: p.skan.name,
            area: p.skan._turfAreaHa || p.skan.area || 0,
            boundary: p.skan.boundary,
            status: "matched",
          })),
          ...basicallyIdenticalPairs.map((p) => ({
            key: "bi-" + p.skan.field_id,
            name: p.skan.name,
            area: p.skan._turfAreaHa || p.skan.area || 0,
            boundary: p.skan.boundary,
            status: "matched",
          })),
          ...autoImportedSkan.map((f) => ({
            key: "as-" + f.field_id,
            name: f.name,
            area: f._turfAreaHa || f.area || 0,
            boundary: f.boundary,
            status: "imported-skan",
          })),
          ...autoImportedAw.map((awB) => {
            const awField = agworldFields.find((f) => f.id === awB.field_id)
            return {
              key: "aw-" + awB.field_id,
              name: awField?.attributes?.name || awB.field_id,
              area: awB._turfAreaHa || awB.area_scalar || 0,
              boundary: awB.boundary,
              status: "imported-agworld",
            }
          }),
        ].sort((a, b) => a.name.localeCompare(b.name))}

        {#if resolvedCombined.length > 0}
          <details class="mb-3 rounded-lg bg-base-200/40">
            <summary
              class="flex cursor-pointer select-none items-center gap-2 px-3 py-2 text-sm font-semibold"
            >
              <Icon
                icon="solar:check-circle-bold"
                width="18"
              ...
              />
              <span
                >{resolvedCombined.length} fields resolved automatically</span
              >
              <span class="text-xs font-normal opacity-50"
                >— matched &amp; imported, no action needed</span
              >
            </summary>
            <div
              class="grid max-h-[260px] grid-cols-2 gap-1.5 overflow-y-auto px-3 pb-3 sm:grid-cols-3 lg:grid-cols-4"
            >
              {#each resolvedCombined as item (item.key)}
                ...
                <div
                  class="flex items-center gap-1.5 rounded bg-base-100/60 p-1.5"
                >
                  <FieldOutlineThumbnail
                    ...
                    outlineColor={statusStyle.outline}
                  />
                  <div class="min-w-0 flex-1">
                  ...
                  <Icon
                    ...
                    width="12"
                    height="12"
                    class="{statusStyle.text} shrink-0"
                  />
                </div>
              {/each}
            </div>
        ...
        {#if reviewSkan.length > 0 || reviewAw.length > 0}
          {#if reviewOverlayDismissed}
            <button
              class="agw-btn agw-btn-accent agw-btn-sm mb-5"
              on:click={() => (reviewOverlayDismissed = false)}
            >
        ...
        {#if showReviewOverlay}
          ...
          {@const awId =
            proposalDominant?.source === "agworld"
              ? proposalDominant.id
              : proposalCandidates[0]?.id}
          {@const skanField = agskanFields.find((f) => f.field_id === skanId)}
          {@const awB = agworldBoundaries.find((b) => b.field_id === awId)}
          ...
          {@const isSplitAction = proposalDominant
            ? dominantSideChoice !== proposalDominant.source
            : false}
          {@const bottomBarArea =
            proposalType === "match"
              ? boundaryChoice === "agskan"
                ...
                : awArea
              : domArea}
          {@const bottomBarBoundary =
            proposalType === "match"
              ? boundaryChoice === "agskan"
                ? skanField?.boundary
                : awB?.boundary
              : domBoundary}
          {@const bottomBarNameDup =
            proposalType && proposalType !== "split"
              ? findDuplicateField(finalFieldName)
              : null}

          <ConsolidationShell
            onBack={() => (reviewOverlayDismissed = true)}
            countLabel="{reviewSkan.length + reviewAw.length} remaining"
            fullscreen={false}
          ...
          >
            <svelte:fragment slot="sidebar">
              <div class="flex-1 overflow-y-auto p-2">
                ...
                {:else if proposalType === "match"}
                  ...
                  <div
                    class="mb-1.5 px-0.5 text-[9px] font-semibold uppercase tracking-wide text-neutral-500 sm:text-[10px]"
                  >
                    Choose boundary
                  </div>
                  <div class="flex flex-col gap-1.5">
                    <button
                      class="flex flex-col items-center gap-1 rounded-lg border-2 p-1.5 transition-all sm:flex-row sm:gap-2 sm:p-2 {boundaryChoice ===
                      'agskan'
                      ...
                      on:click={() => {
                        boundaryChoice = "agskan"
                        nameChoice = "agskan"
                        finalFieldName = skanName
                        mapFocus = { id: skanId, source: "agskan" }
                      }}
                    >
                      <FieldOutlineThumbnail
                        boundary={skanField?.boundary}
                        width={40}
                        height={28}
                        fillColor={boundaryChoice === "agskan"
                          ? "#06b6d4"
                          : "#3a1a0a"}
                        outlineColor={boundaryChoice === "agskan"
                          ? "#fff"
                          : "#f97316"}
                      />
                      <div class="min-w-0 flex-1 text-center sm:text-left">
                        <div
                          class="text-[9px] font-semibold text-neutral-400 sm:text-[10px]"
                        >
                          AgSKAN
                        </div>
                        <div
                          class="truncate font-mono text-[10px] text-neutral-200 sm:text-[11px]"
                        >
                          {skanName}
                        </div>
                        <div class="text-[9px] text-neutral-500 sm:text-[10px]">
                          {skanArea.toFixed(1)} ha
                        </div>
                      </div>
                      {#if boundaryChoice === "agskan"}<Icon
                    ...
                    <button
                      ...
                      'agworld'
                        ...
                        : 'border-neutral-700 hover:border-neutral-500'}"
                      on:click={() => {
                        boundaryChoice = "agworld"
                        nameChoice = "agworld"
                        finalFieldName = awName
                        mapFocus = { id: awId, source: "agworld" }
                      }}
                    >
                      <FieldOutlineThumbnail
                        boundary={awB?.boundary}
                        width={40}
                        height={28}
                        fillColor={boundaryChoice === "agworld"
                          ? "#06b6d4"
                          : "#3a1a0a"}
                        outlineColor={boundaryChoice === "agworld"
                          ? "#fff"
                          : "#f97316"}
                      />
                      <div class="min-w-0 flex-1 text-center sm:text-left">
                      ...
                      {#if boundaryChoice === "agworld"}<Icon
                          icon="solar:crown-bold"
                          width="14"
                          height="14"
                          class="shrink-0 text-cyan-400"
                ...
                {:else if proposalType === "split"}
                  ...
                  <div class="mb-3 flex flex-col gap-1.5">
                    <button
                      class="flex items-center justify-between rounded-lg border-2 p-1.5 transition-all sm:p-2 {skanWins
                        ? 'border-cyan-500 bg-cyan-900/20'
                        : 'border-neutral-700 hover:border-neutral-500'}"
                      on:click={() => {
                        dominantSideChoice = "agskan"
                    ...
                    >
                      <span
                        class="text-[10px] font-semibold text-neutral-200 sm:text-xs"
                        >AgSKAN</span
                      >
                      <span class="text-[9px] text-neutral-400 sm:text-[10px]"
                        >{agskanCount} kept</span
                      >
                    </button>
                    <button
                      class="flex items-center justify-between rounded-lg border-2 p-1.5 transition-all sm:p-2 {!skanWins
                    ...
                    {#each proposalCandidates as cand, ci (cand.id + "-" + ci)}
                      {@const candBoundary =
                        cand.source === "agskan"
                          ? agskanFields.find((f) => f.field_id === cand.id)
                              ?.boundary
                          : agworldBoundaries.find(
                              (b) => b.field_id === cand.id,
                            )?.boundary}
                      {@const isSelected = selectedOverlayIds.includes(cand.id)}
                      <button
                        class="flex w-full flex-col items-center gap-1 rounded p-1.5 text-left sm:flex-row sm:gap-2 {isSelected
                          ? 'bg-cyan-900/40 ring-1 ring-cyan-500'
                          : 'opacity-50 hover:bg-neutral-800'}"
                      ...
                      >
                        <FieldOutlineThumbnail
                          ...
                          width={32}
                          height={22}
                          fillColor={isSplitAction ? "#06b6d4" : "#3a1a0a"}
                          outlineColor={isSplitAction ? "#fff" : "#f97316"}
                        />
                        <div class="min-w-0 flex-1 text-center sm:text-left">
                          <div
                            class="truncate font-mono text-[10px] text-neutral-200 sm:text-[11px]"
                          ...
                          <div
                            class="text-[9px] text-neutral-500 sm:text-[10px]"
                          >
                            {cand.areaHa.toFixed(1)} ha
                          </div>
                        </div>
                        {#if isSelected}<Icon
                            icon="solar:check-circle-bold"
                ...
                {:else}
                  ...
                  <div
                    class="mb-3 flex flex-col items-center gap-1 rounded-lg border-2 border-cyan-500 bg-cyan-900/20 p-1.5 sm:flex-row sm:gap-2 sm:p-2"
                  >
                    <FieldOutlineThumbnail
                      boundary={domBoundary}
                      width={40}
            ...
            <svelte:fragment slot="bottombar">
              {#if proposalType === "split" && isSplitAction}
                <div
                  class="mx-auto max-h-[140px] max-w-2xl space-y-1.5 overflow-y-auto px-4 py-2"
                >
                      ...
                      {#if selectedOverlayIds.includes(cand.id)}
                        ...
                        <div>
                          <div class="flex min-w-0 items-center gap-1.5">
                            ...
                            <input
                              type="text"
                              class="input input-xs input-bordered min-w-0 flex-1 bg-neutral-800 font-mono text-neutral-100 {isDup
                                ? 'border-red-500'
                                : 'border-neutral-600'}"
                              bind:value={splitFieldNames[ci]}
                          ...
                          {#if isDup}<div
                            ...
                            >
                              Name already used elsewhere
                            </div>{/if}
                        </div>
                      {/if}
                    {/each}
                  {:else}
                    <div class="py-1 text-center text-xs text-neutral-500">
                      Select fields in sidebar
                    </div>
                  {/if}
                </div>
                <div class="mx-auto max-w-2xl px-4 pb-3 sm:pb-4">
                  <button
                    class="btn btn-success btn-sm w-full whitespace-nowrap"
              ...
              {:else if proposalType}
                <div class="mx-auto max-w-2xl px-4 py-3 sm:py-4">
                  <div class="mb-1 flex min-w-0 items-center gap-2">
                    ...
                    <input
                      type="text"
                      class="input input-sm input-bordered min-w-0 flex-1 bg-neutral-800 font-mono text-neutral-100 {bottomBarNameDup
                        ? 'border-red-500'
                        : 'border-neutral-600'}"
                      bind:value={finalFieldName}
                      placeholder="Field name"
                    />
                    <span
                      class="shrink-0 whitespace-nowrap font-mono text-xs text-neutral-400"
                      >{bottomBarArea.toFixed(1)} ha</span
                    >
                  </div>
                  {#if bottomBarNameDup}
                    <div class="mb-1.5 text-[10px] text-red-400">
                      Name already used by another field
                    </div>
                  {/if}
                  <button
                    class="btn btn-success btn-sm w-full whitespace-nowrap"
                    disabled={proposalType === "split" &&
                      isSplitAction &&
        ...
        {#if consolidatedFields.length > 0}
          <details class="mb-6">
            <summary class="mb-3 cursor-pointer text-lg font-semibold">
              <Icon
              ...
              <span class="ml-2 text-sm opacity-50"
                >({consolidatedFields.length} fields)</span
              >
            </summary>
            <div class="max-h-[400px] overflow-auto">
              <table class="table table-xs">
                <thead>
                  <tr
                    ><th>Name</th><th>Area</th><th>Category</th><th>Name src</th
                    ><th>Geom src</th><th>AgSKAN</th><th>Agworld</th></tr
                  >
                </thead>
                <tbody>
                  {#each consolidatedFields as cf (cf.id)}
                    <tr>
                      ...
                      <td class="text-xs">
                        {#if cf.agskan.keep.length}keep: {cf.agskan.keep.join(
                            ", ",
                          )}{/if}
                        {#if cf.agskan.create}<span class="text-success"
                            >create</span
                          >{/if}
                        {#if cf.agskan.delete.length}<span class="text-error"
                            >del: {cf.agskan.delete.join(", ")}</span
                          >{/if}
                      </td>
                      <td class="text-xs">
                        {#if cf.agworld.keep.length}keep: {cf.agworld.keep.join(
                            ", ",
                          )}{/if}
                        {#if cf.agworld.create}<span class="text-success"
                            >create</span
                          >{/if}
                        {#if cf.agworld.delete.length}<span class="text-error"
                            >del: {cf.agworld.delete.join(", ")}</span
                          >{/if}
                      </td>
                    </tr>
                  {/each}
          ...
          <div class="mb-6 flex flex-wrap gap-2">
            <button
              class="agw-btn agw-btn-ghost agw-btn-sm"
              on:click={() => (showConsolidatedPreview = true)}
            >
              <Icon
                icon="solar:map-point-bold"
                width="14"
            ...
            <button
              class="agw-btn agw-btn-primary agw-btn-sm"
              on:click={saveConsolidationMetadata}
              disabled={savingConsolidated}
            >
              <Icon
            ...
            <button
              class="agw-btn agw-btn-danger agw-btn-sm"
              on:click={undoConsolidationSave}
              disabled={savingConsolidated}
            >
              {#if savingConsolidated}
          ...
          {#if saveResults}
            <div
              class="alert {saveResults.errors.length > 0
                ? 'alert-warning'
                : 'alert-success'} mb-6 text-sm"
            >
              <Icon
                icon={saveResults.errors.length > 0
              ...
              <div>
                ...
                {#if saveResults.errors.length > 0}
                  <p class="mt-1 text-xs opacity-70">
                    {saveResults.errors.length} errors:
                  </p>
                  <ul class="list-inside list-disc text-xs opacity-70">
                    {#each saveResults.errors as err, i (i)}<li
                        class="truncate"
                      >
                        {err}
                      </li>{/each}
        ...
        {#if identicalPairs.length === 0 && basicallyIdenticalPairs.length === 0 && autoImportedSkan.length === 0 && autoImportedAw.length === 0 && reviewSkan.length === 0 && reviewAw.length === 0 && consolidatedFields.length === 0}
          <div class="card mb-6 bg-base-200">
            <div class="card-body p-6 text-center">
              ...
              <p class="mt-1 text-xs opacity-30">
                {agskanFields.length} AgSKAN · {agworldFields.length} Agworld
              </p>
            </div>
          </div>
        {/if}
  ...
  {#if showConsolidatedPreview}
    {@const sortedFields = [...consolidatedFields].sort((a, b) =>
      a.name.localeCompare(b.name),
    )}
    {@const previewField = sortedFields.find((f) => f.id === selectedPreviewId)}
    {@const duplicateIds = new Set(
      sortedFields
        .filter((f) => findDuplicateField(f.name, f.id))
        .map((f) => f.id),
    )}
    {@const filteredFields = previewSearchQuery.trim()
      ? sortedFields.filter((f) =>
          f.name
            .toLowerCase()
            .includes(previewSearchQuery.trim().toLowerCase()),
        )
      : sortedFields}
    ...
    <ConsolidationShell
      onBack={() => (showConsolidatedPreview = false)}
      countLabel="{sortedFields.length} fields"
    >
      <svelte:fragment slot="sidebar">
        <!-- Search bar, stickied to top -->
        <div
          class="sticky top-0 z-10 border-b border-neutral-700 bg-neutral-900 p-1.5"
        >
          <div class="relative">
            <Icon
              icon="solar:magnifer-linear"
              width="12"
              height="12"
              class="absolute left-1.5 top-1/2 -translate-y-1/2 text-neutral-500"
            />
            <input
              type="text"
              placeholder="Search"
              class="input input-xs w-full border-neutral-600 bg-neutral-800 pl-5 text-[10px] text-neutral-200 sm:text-[11px]"
        ...
        <div class="flex flex-1 flex-col gap-0.5 overflow-y-auto p-1.5">
          {#if reviewFields.length > 0}
            <div
              class="flex items-center gap-1 px-1 pb-0.5 pt-0.5 text-[9px] font-semibold uppercase tracking-wide text-red-400 sm:text-[10px]"
            >
              <Icon icon="solar:danger-triangle-bold" width="10" height="10" /> Needs
              review
            </div>
            {#each reviewFields as f (f.id)}
              {@const isSelected = selectedPreviewId === f.id}
              <button
                class="flex w-full flex-col items-center gap-1 rounded p-1.5 text-left sm:flex-row sm:items-center sm:gap-1.5 {isSelected
                  ? 'bg-cyan-900/30 ring-1 ring-cyan-700/50'
              ...
              >
                <div class="relative shrink-0">
                  <FieldOutlineThumbnail
                    ...
                    width={32}
                    height={22}
                    fillColor={isSelected ? "#06b6d4" : "#64748b"}
                    outlineColor={isSelected ? "#67e8f9" : "#94a3b8"}
                  />
                  <Icon
                    icon="solar:danger-triangle-bold"
                    width="11"
                    height="11"
                    class="absolute -right-1 -top-1 rounded-full bg-neutral-900 text-red-400"
                  />
                </div>
                <div class="min-w-0 flex-1 text-center sm:text-left">
                  <div
                    class="truncate font-mono text-[10px] leading-tight sm:text-[11px] {isSelected
                      ? 'text-cyan-300'
                      : 'text-red-300'}"
                  >
                    {f.name}
          ...
          {#if filteredFields.length === 0}
            <div class="py-3 text-center text-[10px] text-neutral-500">
              No fields match
            </div>
          {/if}
        </div>
      ...
      <svelte:fragment slot="map">
        <ConsolidationMap
          agskanFields={sortedFields.map((f) => ({
            field_id: f.id,
            name: f.name,
            boundary: f.boundary,
            _turfAreaHa: f.area_ha,
          }))}
          agworldBoundaries={[]}
          focusField={null}
          ...
          on:selectField={(e) => {
            ...
            if (field) {
              editingPreviewName = field.name
              editingPreviewArea = field.area_ha
            }
          }}
        />
      </svelte:fragment>
      ...
      <svelte:fragment slot="bottombar">
        {#if previewField}
          {@const duplicateField = editingPreviewName
            ? findDuplicateField(editingPreviewName, selectedPreviewId)
            : null}
          <div class="mx-auto max-w-2xl px-4 py-3 sm:py-4">
            <div class="flex items-center gap-3">
              <!-- Thumbnail -->
              <FieldOutlineThumbnail
                boundary={previewField.boundary}
                width={48}
                height={48}
                fillColor="#0080ff"
                outlineColor="#bfffbf"
              ...
              <div class="min-w-0 flex-1">
                <label
                  class="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-neutral-500"
                  >Field name</label
                >
                <input
                  type="text"
                  bind:this={previewNameInputEl}
                  class="input input-sm input-bordered w-full bg-neutral-800 font-mono text-neutral-100 sm:input-md {duplicateField
              ...
              <div class="shrink-0">
                <label
                  class="mb-1 block text-right text-[10px] font-semibold uppercase tracking-wide text-neutral-500"
                  >Area (ha)</label
                >
                <input
                  type="number"
                  ...
                  class="input input-sm input-bordered w-[80px] border-neutral-600 bg-neutral-800 text-right font-mono text-neutral-100 sm:input-md"
                  bind:value={editingPreviewArea}
                  on:input={() => {
                    editingPreviewArea =
                      Math.round(editingPreviewArea * 10) / 10
                  }}
                  on:keydown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault()
              ...
              <div class="flex shrink-0 flex-col justify-end">
                <button
                  class="btn btn-ghost btn-sm text-red-400 hover:bg-red-900/20 hover:text-red-300"
            ...
            <div class="mt-2 flex min-h-[16px] items-center justify-between">
              {#if duplicateField}
                <button
                  class="flex items-center gap-1 text-[11px] text-red-400 hover:text-red-300"
                  on:click={() => {
                    selectedPreviewId = duplicateField.id
                    editingPreviewName = duplicateField.name
                    editingPreviewArea =
                      Math.round(duplicateField.area_ha * 10) / 10
                  }}
                >
                  <Icon
                    icon="solar:danger-triangle-bold"
              ...
              {:else}
                <span class="text-[11px] text-neutral-500">
                  Using {previewField.name_source === "agskan"
                    ? "AgSKAN"
                    : "Agworld"} name · {previewField.geometry_source ===
                  "agskan"
                    ? "AgSKAN"
                    : "Agworld"} boundary
                </span>
              ...
              {#if savedFlash}
                <span
                  class="flex items-center gap-1 text-[11px] text-green-400"
...
<style>
  ...
  .agworld-records {
    /* No extra padding/max-width — the outer .agw-page wrapper handles it */
  }
  ...
  .year-chip:hover:not(.active) {
    color: oklch(var(--contrast-content) / 0.85);
  }
  ...
  .field-group-header:hover {
    background: oklch(var(--base-300) / 0.3);
  }
  .field-group-info {
    flex: 1;
    min-width: 0;
  ...
  .activity-btn.loading {
    opacity: 0.7;
    pointer-events: none;
  }
  .activity-btn.has-data {
  ...
  .activity-type-badge.actual {
    background: rgba(34, 197, 94, 0.12);
    color: #22c55e;
  }
  .activity-type-badge.work-order {
    background: rgba(245, 158, 11, 0.12);
    color: #f59e0b;
  }
  .activity-type-badge.plan {
    background: rgba(59, 130, 246, 0.12);
    color: #60a5fa;
  }
  .activity-type-badge.discarded-plan {
  ...
  /* Modal content styles (DaisyUI handles backdrop, box, header) */
  .modal-section {
    display: flex;
    flex-direction: column;
    gap: 4px;
  ...
  /* Activity modal - alternating table rows */
  .modal-box table tbody tr:nth-child(even) {
    background: rgba(128, 128, 128, 0.1);
  }
  .modal-box table tbody tr:hover {
    background: rgba(128, 128, 128, 0.18);
  }

  /* Layout variants: Hero card (green) */
  .al-card-green {
    background: linear-gradient(
      135deg,
      oklch(var(--success) / 0.08),
      oklch(var(--success) / 0.03)
    );
  ...
  .al-field-icon-ring {
    box-shadow: 0 0 0 4px oklch(var(--success) / 0.08);
  }
  ...
  .al-crop-dot {
    ...
    height: 6px;
    border-radius: 50%;
    background: oklch(var(--success) / 0.6);
    flex-shrink: 0;
  }
  ...
  .al-timeline-line {
    ...
    bottom: 6px;
    width: 2px;
    background: oklch(var(--contrast-content) / 0.12);
    border-radius: 1px;
  }
  .al-timeline-sm .al-timeline-line {
    left: 4px;
  }
  .al-timeline-row {
    position: relative;
  ...
  .al-timeline-sm .al-timeline-row {
    padding: 4px 0 5px;
  }
  .al-timeline-dot,
  .al-timeline-dot-dashed {
    position: absolute;
    left: -15px;
    top: 9px;
    width: 8px;
  ...
  .al-timeline-sm .al-timeline-dot,
  .al-timeline-sm .al-timeline-dot-dashed {
    left: -12px;
    top: 7px;
    width: 6px;
    height: 6px;
  }
  .al-timeline-dot.bg-success {
    background: oklch(75% 0.15 140);
    box-shadow: 0 0 0 3px oklch(var(--success) / 0.2);
  }
  .al-timeline-dot.bg-warning {
    background: oklch(80% 0.12 80);
    box-shadow: 0 0 0 3px oklch(var(--warning) / 0.2);
  }
  .al-timeline-dot-dashed {
    background: transparent;
    border: 2px dashed oklch(var(--contrast-content) / 0.2);
    box-shadow: none;
  }
  ...
  .lifecycle-badge.plan {
    background: rgba(59, 130, 246, 0.12);
    color: #60a5fa;
  }
  .lifecycle-badge.actual {
    background: rgba(34, 197, 94, 0.12);
    color: #22c55e;
  }
  .lifecycle-badge.discarded-plan {
  ...
  .activity-row.discarded-converted {
  ...
  .agw-btn-primary:hover:not(:disabled) {
    background: rgba(37, 99, 235, 0.18);
  }
  .agw-btn-accent {
  ...
  .agw-btn-accent:hover:not(:disabled) {
  ...
  .agw-btn-danger:hover:not(:disabled) {
    background: rgba(220, 38, 38, 0.15);
  }
  .agw-btn-ghost {
    color: oklch(var(--contrast-content) / 0.5);
  ...
  .acct-card:hover {
    ...
    border-color: oklch(var(--contrast-content) / 0.15);
  }
  .acct-card-name {
    font-size: 17px;
    font-weight: 700;
  ...
  .query-btn:hover {
    color: oklch(var(--contrast-content) / 0.95);
    border-color: rgba(59, 130, 246, 0.4);
    background: oklch(var(--base-200) / 0.5);
  }
  .query-btn.loading {
  ...
  .log-summary:hover {
    background: oklch(var(--base-100) / 0.5);
  }
  .log-detail {
