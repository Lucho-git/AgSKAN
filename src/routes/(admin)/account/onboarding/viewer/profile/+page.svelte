<!-- src/routes/(admin)/account/onboarding/viewer/profile/+page.svelte -->
<script lang="ts">
  ...
  $: isFormValid = fullName.trim().length > 0 && (skipMapId || hasConnectedMap)
  ...
  async function handleContinue() {
    ...
    try {
      ...
      goto("/account/onboarding/viewer/vehicle")
    } catch (error) {
