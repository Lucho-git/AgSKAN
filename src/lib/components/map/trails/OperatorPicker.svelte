...
<script lang="ts">
  ...
  async function handleSelect(operatorId: string) {
    ...
    saving = false
    if (op) {
      toast.success(`Operator: ${op.name}`)
      dispatch("selected", op)
    } else {
      toast.error("Failed to select operator")
    }
  }
  ...
  function confirmTakeOperator() {
    ...
    const op = pendingTakeOp
    pendingTakeOp = null
    handleSelect(op.id)
  }

  function cancelTakeOperator() {
    pendingTakeOp = null
  }
  ...
  async function handleCreate() {
    ...
    saving = false
    if (op) {
      toast.success(`Operator created: ${op.name}`)
      dispatch("selected", op)
    } else {
  ...
  <div class="op-modal">
    <div class="op-header">
      ...
      <div class="op-header-text">
        ...
        <p>
          {context === "start"
            ? "Choose who is driving for this session"
            : "Select an operator for this record"}
        </p>
      </div>
      <button
        type="button"
    ...
    <div class="op-body">
      ...
      {#if $operatorStore?.operator}
        <button
          type="button"
          class="op-current-box has-operator"
          on:click={() => dispatch("close")}
        >
          <div
            class="op-current-avatar"
      ...
      {:else}
        <div class="op-current-box">
          <div class="op-current-avatar op-current-avatar-empty">
            <User size={18} />
          </div>
      ...
      {#if loading}
        <div class="op-loading">
          <Loader2 size={24} class="spin" /><span>Loading operators…</span>
        </div>
      {:else if mode === "list"}
        {@const filtered = operators.filter(
          (op) => op.id !== $operatorStore?.operator?.id,
        )}
          ...
          <div class="op-list">
            {#each filtered as op (op.id)}
              <button
                type="button"
                class="op-option"
                class:in-use={op.in_use_by}
                disabled={saving}
                on:click={() => handleOneTap(op)}
              >
                <div class="op-avatar" style="--op-color: {op.color}">
                  {getInitials(op.name)}
                </div>
                <span class="op-name">{op.name}</span>
                {#if op.in_use_by}<span class="op-inuse-label"
                    >In use by {op.in_use_by}</span
                  >{/if}
        ...
        {:else if operators.length === 0}
          <div class="op-empty">
            <p>No operators on this map yet.</p>
            <p class="op-empty-sub">Create one below to get started.</p>
          </div>
        {/if}
        <button
          type="button"
          class="op-create-toggle"
      ...
      {:else}
        <div class="op-create-form">
          <label for="op-name-input" class="op-label">Operator Full Name</label>
          <input
            id="op-name-input"
            type="text"
            bind:value={newName}
          ...
          />
          <p class="op-hint">
            This name will be saved to records while active.
          </p>
          <div class="op-create-actions">
            {#if operators.length > 0}
              <button
                type="button"
                class="op-back-btn"
                on:click={() => (mode = "list")}>Back</button
              >
            {/if}
            <button
              type="button"
              class="op-confirm-btn"
...
{#if pendingTakeOp}
  <div class="op-confirm-overlay" on:click|self={cancelTakeOperator}>
    <div class="op-confirm-dialog">
      ...
      <p>
        <strong>{pendingTakeOp.name}</strong> was last being used by
        <strong>{pendingTakeOp.in_use_by}</strong>.
      </p>
      <div class="op-confirm-buttons">
        <button
          type="button"
          class="op-confirm-cancel"
          on:click={cancelTakeOperator}>Cancel</button
        >
        <button
          type="button"
          class="op-confirm-take"
          disabled={saving}
          on:click={confirmTakeOperator}
        >
...
<style>
  ...
  .op-current-box.has-operator {
    background: rgba(74, 222, 128, 0.06);
    border-color: rgba(74, 222, 128, 0.2);
    cursor: pointer;
    transition: all 0.15s ease;
  }
  .op-current-box.has-operator:hover {
  ...
  .op-create-form input:focus {
    border-color: rgba(96, 165, 250, 0.5);
  }
  .op-create-form input::placeholder {
    color: rgba(255, 255, 255, 0.3);
  }
  .op-hint {
    margin: 0;
  ...
  .op-confirm-btn:hover:not(:disabled) {
  ...
  .op-confirm-take:hover:not(:disabled) {
    background: rgba(96, 165, 250, 0.3);
  }
  .op-confirm-take:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }
