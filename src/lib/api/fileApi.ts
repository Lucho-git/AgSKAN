...
export const fileApi = {
  ...
  async deleteFile(fileName: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        message:
          error.message ||
          "Error deleting file. If this persists please contact us.",
      }
    }
  },
  ...
  async updateField(
    ...
    updates: {
      ...
      polygonAreas?: { individual_areas: number[]; total_area: number } | null
      icon?: string
      color?: string
      field_type?: string
      farm_id?: string
    ...
    try {
      ...
      if (updates.boundary !== undefined) updateData.boundary = updates.boundary
      ...
      if (updates.polygonAreas === null) {
        updateData.polygon_areas = null
        if (updates.area !== undefined) {
          updateData.area = updates.area
        }
      } else if (updates.polygonAreas?.individual_areas?.length > 0) {
        const calculatedTotalArea =
          updates.polygonAreas.individual_areas.reduce(
            (sum, area) => sum + area,
            0,
          )
    ...
    try {
      ...
      try {
        ...
        if (copyError) {
          ...
          console.log(
            "Main upload succeeded, but backup failed. Check bucket permissions.",
          )
        } else {
  ...
  async loadFields() {
    try {
      ...
      return { fields: [], error: describeApiError(error) }
    }
  },
}
