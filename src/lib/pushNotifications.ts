import {
  PUBLIC_VAPID_APPLICATION_SERVER_KEY,
...
function getDeviceType() {
  ...
  if (/iPad|iPhone|iPod/.test(ua) && !window.MSStream) {
    return /iPad/.test(ua) || Math.max(screenWidth, screenHeight) > 1000
      ? "iOS Tablet"
      : "iPhone"
  }
    ...
    request.onsuccess = (event) => {
      ...
      getRequest.onsuccess = () => {
        ...
        if (getRequest.result) {
          ...
          if (
            typeof getRequest.result === "object" &&
            getRequest.result.id &&
            getRequest.result.type
          ) {
  ...
  if ("serviceWorker" in navigator && "PushManager" in window) {
    try {
      ...
      const applicationServerKey = new Uint8Array(
        JSON.parse(PUBLIC_VAPID_APPLICATION_SERVER_KEY),
      )
      ...
      if (existingSubscription) {
        ...
        if (!arrayBufferEqual(existingKey, applicationServerKey)) {
          console.log(
            "Existing subscription uses different key, unsubscribing...",
          )
...
export async function sendPushNotification(subscription, title, body) {
  ...
  try {
    const response = await fetch(
      "https://hmxxqacnzxqpcheoeidn.supabase.co/functions/v1/send-notification",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
